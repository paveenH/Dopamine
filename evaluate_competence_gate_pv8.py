#!/usr/bin/env python3
"""pv8 competence-gate wrapper. LOADER ONLY -- the rules are not re-implemented.
...
`mdf_*/`, which pv8 deliberately does not produce: disguising a pv8 file as pv6
would make two non-poolable protocols silently poolable.

So this file supplies a pv8 reader and calls the SAME frozen `evaluate()`. The
...
The environment is genuinely unchanged between pv6, pv7 and pv8, which is what makes
...
-------------------------------------------------------------------------
A pv8 Easy-bare pass shows Bandit competence UNDER A STRUCTURED,
...
fails and the Policy that Stage 2 reads is recovered by an extractor. pv8 adds
a CHOICE HISTORY block to Stage 1, so a pass is additionally conditional on
that scaffold. Any writeup carries both qualifiers.

pv8, like pv7, separates two things pv6 could not:
...
Usage
-----
    python3.10 evaluate_competence_gate_pv8.py --result .../pv8_easy_bare
"""
...
def load_pv8_dir(path: str):
    """Read pv8 result JSONs. alpha=0 only -- the gate is an alpha=0 statement.
    ...
    """
    files = sorted(glob.glob(os.path.join(path, "**", "bandit_pv8_*.json"),
                             recursive=True))
    if not files:
        stray = glob.glob(os.path.join(path, "**", "bandit_pv6_*.json"),
                          recursive=True)
        raise SystemExit(
            f"no pv8 result JSON under {path}"
...
def interface_report(runs: list[dict]) -> dict:
    ...
    Reported so a verdict has an address: a pv8 failure can now be attributed
...
def main() -> int:
    ...
    ap.add_argument("--result", action="append", default=[],
                    help="pv8 result dir (repeatable)")
    ...
    for path in args.result:
        for d, env, f in load_pv8_dir(path):
            runs = d["runs"]
            bank = br.build_seed_bank(env)
            label = f"pv8 {os.path.basename(path)} [{os.path.basename(f)}]"
