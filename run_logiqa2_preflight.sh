...
case "$MODEL" in
  llama3)
    SIZE=8B
    MODEL_DIR="${MODEL_DIR:-$WORK_DIR/models/Llama-3.1-8B-Instruct}"
    MASK="$WORK_DIR/components/mask/llama3_nmd_logits/nmd_0.5_11_20_8B.npy"
    CONFIGS=(0-11-20 neg6-11-20) ;;
  qwen2.5)
    SIZE=7B
    MODEL_DIR="${MODEL_DIR:-$WORK_DIR/models/Qwen2.5-7B-Instruct}"
    MASK="$WORK_DIR/components/mask/qwen2.5_nmd_logits/nmd_0.5_16_22_7B.npy"
...
"$PY" get_answer_logiqa2_preflight.py \
  ...
  --configs "${CONFIGS[@]}" \
  --preflight_file "$BENCH/logiqa2_p4_preflight.json" \
