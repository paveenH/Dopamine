...
- **Betting: check the per-sample CSV header before citing a paired p-value.** McNemar/Wilcoxon in `AdaDopamine.md` §3.1 pair by `sample_idx`, which the pre-2026-07-29 CSVs lack (Llama GPQA, Llama MMLU, Qwen MMLU — only Qwen GPQA has it). Re-runs of those three add the column plus `acc_explicit_pct`/`ans_fallback_rate`/`orig_rows`. This is a SCHEMA gap, not wrong numbers. Betting also has **no fixed seed** (`temperature=1.0`), so any re-run resamples — update the §3.1 tables afterwards instead of treating the delta as a regression.
