...
# -*- coding: utf-8 -*-
"""
MATH open-ended generation evaluation with neuron editing (regenerate).

MATH analogue of get_answer_regenerate_gsm8k.py — same architecture, only the
task-specific bits differ (build_math_suite templates, \\boxed{} extraction).

- ALL conditions go through vc.regenerate() (prefill-only steering). alpha=0 is
  a no-op (diff = mask×0), so the baseline shares the exact generation path with
  ±4 steering — avoids the generate-vs-regenerate ~2% padding gap (see CLAUDE.md).
- Roles: neutral → neutral template; others → neg ("Now you are {character}."),
  NO "honest" framing (MATH has no E-option), via utils.ROLE_TO_CHARACTER.

Answer extraction: \\boxed{...} (balanced-brace) → last number → last line, then
is_correct_math (normalized string equality, then sympy symbolic equivalence).
Ground truth is the full solution text; we extract the last \\boxed{} from it.
"""
