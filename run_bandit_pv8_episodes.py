#!/usr/bin/env python3
"""pv8 episode driver. Thin: loads model + mask, loops seeds, writes JSON.

pv8 = pv7 + a CHOICE HISTORY block in the Stage-1 prompt. Everything
protocol-specific lives in `bandit_pv8_episode`, which reuses pv7's episode
loop and overrides only that prompt; Stage 2 is byte-unchanged.

OUTPUT NAMING -- pv8 files are never disguised as pv7 or pv6
------------------------------------------------------------
Files are `bandit_pv8_{env}_{size}_{ls}_{le}.json` under a pv8-only directory.
The frozen gate loader globs `bandit_pv6_*.json` and the pv7 wrapper globs
`bandit_pv7_*.json`, so pv8 results are invisible to both by construction --
which is intended, since the three protocols are not poolable. The gate RULES
are frozen and reusable; the LOADER is not, so pv8 gets its own wrapper
(`evaluate_competence_gate_pv8.py`) calling the same frozen `evaluate()`.

`environment.name` stays `reference_easy`: the environment genuinely did not
change, which is why the frozen seed banks and algorithmic baselines remain
the correct comparison basis for pv8's gate.

ALPHA=0 IS NOT INHERITED
------------------------
H1 changes the Stage-1 prompt, so the stored pv7 alpha=0 cell is not this
protocol's baseline. Run all three alphas here, same seeds, same tapes, one
directory per alpha.

RESUME
------
Keyed on `bandit_pv8_episode.resume_key`, which carries the protocol tag, both
instruction versions, the history-block version and a hash of the seed
CONTENT. A stored episode is reused only when every behaviour-affecting knob
matches.

Usage
-----
    python run_bandit_pv8_episodes.py --reference_environment easy \\
        --seeds 6 12 13 --ans_file .../pv8_easy_bare_smoke --attest
"""
...
import bandit_pv8_episode as ep
...
def main():
    ...
    out_file = out_dir / (f"bandit_pv8_{args.reference_environment}"
    ...
    for i, seed in enumerate(todo, 1):
        t0 = time.time()
        rec = ep.run_pv8_episode(
            vc, diff, seed=seed, env=env,
            rationale_alpha=args.rationale_alpha,
            action_alpha=args.action_alpha,
            # ALWAYS attest the first episode of a cell. The Stage-1/Stage-2
            # history assertions live in run_pv8_episode and only fire when an
            # attestation exists, so an unattended cell would otherwise never
            # verify that the history reached Stage 1 and stayed out of
            # Stage 2. `--attest` additionally stores round 10.
