...
## 2. 行为学发现：Motivation to Engage（"想不想回答"）

**统一框架**：α 调的是模型 **"想不想回答问题"（motivation to engage）** —— 即多巴胺的本义（incentive salience / 行动的能量，ACL 原文："motivation to engage", "energetic motivation to overcome inhibition"）。

- **α=+4（太想回答）** → 急于输出 → 抢答（没想清就答）+ 停不下来（答完还想继续说 → 空转 loop）
- **α=−4（没那么想回答）** → 不急 → 冷静（不抢答）+ 点到为止（答完干脆收口，没有"还想多说"的冲动）

这统一了之前看似矛盾的跨任务现象（见 §2.2 末）：**同一个 "想不想回答"，在判断题（MMLU-E）表现为"选不选 E"，在生成题（GSM8K）表现为"抢不抢答 / 停不停"** —— 任务给什么出口，motivation 就从哪表达。下文 §2.1–2.3 的所有行为签名（抢答、loop、收口）都是这一个旋钮的不同侧面。
...
> 形象说法：**α−4 像"做完题就交卷"，α+4 像"做完题了还在卷子上反复涂改、自言自语'等等这个对吗''答案是240吧''格式该怎么写'，直到打铃被收卷"。** 两者算的内容一样多，差的是**"想不想继续说"（motivation to engage）** —— 这正是 dopamine 调节的东西，不是 thinking 本身。
