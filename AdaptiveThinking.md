...
## 5.9 Predictive and Transfer Validation: Summary

§5.8 识别的 commitment regime 进一步显示出预测与迁移价值：

- 在 held-out GSM8K 题目上，commitment-only predictor 的 AUROC 为 Llama `.687`、Qwen `.749`，均明显优于 entry gain。
- 在 MATH 的 retrospective locked transfer 中，两模型的 steering 方向均预测正确；Qwen 选中真实最佳工作点 `+6`，Llama 在三点局部曲线上选中 `−4`。
- 在前瞻性封存的 GSM-Hard 上，两模型均选中 observed near-optimal workpoint，直接迁移 GSM8K 工作点分别提高 `6.33 pp` 与 `16.33 pp`。
- 固定工作点在 CoT 条件下仍分别提高 Llama `6.00 pp`、Qwen `13.33 pp`；未检出 CoT × steering interaction。
