...
**定位（措辭邊界，不可放寬）：這是一次「受有效劑量窗限制的部分跨模型複製」，不是完整複製。** 有效劑量窗比 Llama 窄得多（Llama v4 clean range −4…+6；Qwen v5 僅 −2…+2，且 desc 的 +2 未通過 gate），且 asc 與 desc 的可用格數不同。任何把本節寫成「CGT-Seq 在 Qwen 上完整複製」的表述都與資料不符。

**為什麼需要 v5（v4 已凍結為邊界證據）。** Qwen 在 v4 下 **α=0 就沒通過 knowing gate**：desc `qdm_major_blue` **1.000** vs `qdm_major_red` **0.37**（asc 1.000 / 0.65）——這是標籤鎖死而非機率使用，且失敗發生在任何 steering 之前。同一 gate 下 **Llama3-8B v4 通過**（asc .709/.816、desc .691/.819，invalid .000），所以 gate 是在區分模型，不是在否定範式。v4 另有一個**已確認的設計混淆**：它在四處固定 blue-first（場景句、機制句、範例、輸出列表＋提問），且 driver 無任何 shuffle，故標籤與位置完全共線，v4 資料**無法**區分「Blue 標籤偏好」與「第一選項偏好」。

**v5 的兩項改動（並列，不可歸因於單一因素）：** ① 逐輪平衡選項順序（每 phase 嚴格 4/4、每 run 32/32，由獨立 seed 派生的 RNG，`make_box_sequence` 位元不變，故兩版面對相同 chest 序列）；② 範例中性化（`7 of the 10 chests are one colour and 3 are the other → 70%/30%`，保留機率規則但移除顏色錨）。**v4→v5 的改善同時包含這兩項，不得歸因於其中任一項。** 場景句仍為 blue-first，是 v5 已知的殘餘線索。

**α=0 校準（N=3，僅供選 prompt，不參與推論）：** 四項 gate 全數通過，`qdm_major_red` 由 v4 的 .306/.65 升至 **.719（desc）/.854（asc）**。歸因表（控制 `major_color` 後）顯示**殘餘偏差主要跟隨 Blue 標籤而非第一位置**：格內位置效應僅 **+0.065 / +0.071**，且完全集中在 major=red 格（major=blue 兩格皆 1.000，天花板無空間）。邊際位置效應 +0.29 **不可引用**——seeds 0–2 恰使 `first_option` 與 `major_color` 同向偏斜（61/35 而非 48/48），50 seeds 檢驗顯示兩 RNG 獨立（一致率 .510），故該偏斜是小樣本巧合而非耦合缺陷。

**劑量帶 pilot（N=5，−6…+6 七格，全部報告）→ 選定 −2/0/+2。** ±4／±6 落選的理由是兩種**性質不同**的失效：+4/+6 的 `qdm_red` 塌到 .269/.106（desc）——格式完好、證據仍在被讀（`qdm_blue` 仍 ≈.99），是 **Blue 標籤先驗被 +α 放大到壓倒證據**；−4/−6 則是 invalid ≈.52–.63，來自顏色步的數字漂移（`Color: 7`），其劑量曲線單調且乾淨（含數字比例 desc .984/.772/.022/.000）。
...
1. **asc −2/0/+2 = 完整通過的三點劑量結果。** 三格 gate 全通過，`accept_step` 嚴格單調，三個配對比較皆 `p≤7.9e−04`（exact paired Wilcoxon，n=20/16）。
2. **desc −2/0 可正式引用，但只有一個有效對比，不構成完整三點劑量帶。** `accept_step` 3.508→1.677，`p=1.9e−06`。
3. **desc +2 僅作 over-steering 邊界。** 其行為變化（`accept_step` 1.048、`step1_rate` .967、`mean_bet` 94.0%）**必須與 `qdm_red = .5031` 同段報告**，不得單獨引用為 wanting 效應。措辭用「**接近隨機水平且未通過 gate**」——per-run bootstrap 95% CI **[.445, .561]** 同時涵蓋 0.50 與 gate 閾值 0.55，故既不能斷言等於隨機，也不能宣稱達標；gate 依預註冊以點估計判定，判 FAIL。
4. **DAI：−2（−12.97 [−19.03, −7.08]）與 0（+70.21 [+65.93, +74.16]）可正式引用；+2（+88.71）僅為診斷值**——它的 desc 半邊落在未通過 gate 的格上，故**不支撐「完整單調展寬」結論**（對比 Llama v4 在六檔上的單調展寬）。

**機制讀法。** +α 在此協議下呈現**知／行分離的天花板**：commitment latency 持續單調響應至 +2（兩條件皆然），但顏色判斷的標籤先驗被 +α 單調放大——`qdm_label_gap` 沿 α 單調（desc .265→.372→.481）。desc 比 asc 先失效，與其 `qdm_red` 基線本就較低一致（.617 vs .811）。

> **解析口徑：** `parse_color` 取首個顏色詞，故「先複述機率、再宣告顏色」的回覆（`Color: 20% blue, 80% red, I choose Red: Red`）會被讀反。v5 pilot 中 2/4480 輪受影響（皆在 asc −2），修正後 `qdm_red` .7733→.7867，**不改變任何 gate 判定**；**N=20 正式資料掃描為 0/7586**，故上表原始口徑無需敏感性修正。driver 已凍結故不修補（2/4480 不影響結論，改動會使 v1–v4 存量資料口徑分叉），掃描器 `scan_cgt_seq_color_misparse.py` 為既定替代方案。
