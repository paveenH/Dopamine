...
- **核心劑量反應複現。** mean_bet 4.02 → 4.89 → 5.14 → 5.13 → 5.60 → 6.28 → **7.06**，Spearman ρ=**0.464**（n=4,496, p=1.4e−238，取 −8…+4 的單調段）；bet10% 由 11.9% 升至 41.2%。與 Llama 同形。
- **wanting–knowing dissociation 複現且更乾淨。** accuracy 在 **−8…+6 全部八格**皆為 32.8–34.7%（全距 **1.86pp**），**每一格 vs orig 的 χ² 皆不顯著**（p=0.52–0.91）——注意這**包含 +6 那個下注塌陷的格子**：下注行為已經退化成常數，答對率卻毫無變化，這是解離最強的一個切片。唯一偏離的 +8（29.6%）是格式解體導致 15.8% 樣本沒輸出答案字母所致，非知識能力下降（其有效子集 acc 反而是 35.8%）。
...
2. **「betting 夠不到右臂」這個一般化被推翻。** 它是 Llama 的性質，不是 betting 範式的性質——Qwen 在單步決策上就能看到右臂過載。合併兩個模型：betting 的兩臂**都**能觀察到失效，只是**哪一臂先崩是模型特異的**，所以跨模型比較時不能沿用同一組 α 假設，也不能從單一模型的臂形推廣到整個範式。
