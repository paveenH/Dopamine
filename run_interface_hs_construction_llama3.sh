...
# ======= Interface Hidden-State Extraction (CONSTRUCTION SPLIT): Llama3 =======
# Runs extract_interface_hidden_states_construction.py for
# Llama-3.1-8B-Instruct on the LARGER GSM8K construction split
...
# Does NOT touch extract_interface_hidden_states.py, run_interface_hs_llama3.sh,
# or any of their (300-question, 3-task) output.
#
# Usage (server, from /data1/paveen/Dopamine):
#   CUDA_VISIBLE_DEVICES=0 nohup bash run_interface_hs_construction_llama3.sh \
#       > logs/interface_hs_construction_llama3.log 2>&1 &
#
# Syntax check only (no GPU/model needed):
#   bash -n run_interface_hs_construction_llama3.sh
#
# Output: components/hidden_states/llama3/gsm8k_construction/
#   chat_8B.h5, chat_8B.manifest.json
#   bare_8B.h5, bare_8B.manifest.json
#
# CRASH-RECOVERY CAVEAT: if this run dies after chat_8B.h5 is written but
# before bare_8B.h5 is, the overwrite guard will REFUSE the next invocation.
# Resuming requires a DELIBERATE manual step: inspect
#   components/hidden_states/llama3/gsm8k_construction/
