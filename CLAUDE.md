...
| ProofWriter-OWA (`proofwriter-owa-v0`) | **⛔ SUSPENDED 2026-09-04 at the α=0 feasibility gate — do NOT run canary, pilot, or any α sweep.** The evaluation interface is not usable yet: llama `parse_fail=90%` / `trunc=100%`, qwen `parse_fail=73.3%`, and BOTH models score **1/30 = 3.33%** overall. Continuing would compare format failure and loop degeneration, not steering. **Frozen wording: "the explicit-CoT configuration did NOT pass feasibility" — NEVER "steering does not work on ProofWriter"** (no non-zero α has ever been run). The α=0 preflight's `No effective workpoint` verdict is **VACUOUS** and must not be cited: a workpoint verdict over a single dose has nothing to compare. Results are SEALED as-is. Resuming requires a small **balanced 3-shot** prompt feasibility test first; if parse failure is still high, the task is formally TERMINATED. Prereg `proofwriter_owa/PREREG_PROOFWRITER_OWA.md` |
