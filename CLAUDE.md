...
| ↳ RR/CC causal steering on MMLU-E (`mmlue-confidence-steering-v0`) | **PREPARED, GPU RUN PENDING (as of 2026-09-08).** Question: does the ORIGINAL Role NMD mask (`nmd_0.5_11_20_8B.npy`) reproduce a stable MMLU-E steering effect (RR), and does the NEW Confidence NMD mask (`confidence_0.5_11_20_8B.npy`, built by `build_confidence_nmd_mask_for_steering.py`, byte-identical to the mask from the analysis line above, sha256 recorded in its manifest) have an INDEPENDENT causal effect on confident/unconfident behavior (CC)? **Deliberately simplified from an earlier RC/CR/random-support cross-steering design** (kept, unused, under `cross_steering_confidence/{get_answer_cross_steering_mmlue.py,run_cross_steering_mmlue.sh,analyze_cross_steering_mmlue.py,build_cross_steering_vectors.py}` for a later stage) — **this stage runs ONLY RR and CC, each mask used AS-IS with no direction/support decomposition and no norm-matching**; do not compare RR-vs-CC magnitude at the same raw α as a strength claim (their raw norms differ) — only direction-consistency and CC's independent causal presence are in scope. Code: `cross_steering_confidence/get_answer_rr_cc_mmlue.py` (standalone, does not import or modify `get_answer_regenerate_logits.py` — that script can only load a mask-named file × integer α, so a new script was needed to load either of exactly two named masks), `run_rr_cc_mmlue.sh`, `analyze_rr_cc_mmlue.py`. Dose set α∈{−4,−2,0,+2,+4}, `tail_len=1`, injection band decoder 11–19, all 57 tasks, both `confident`/`unconfident` prompt roles; α=0 baseline runs once and is shared by RR/CC. **`invalid` was renamed `wrong_non_E`** — the argmax-over-A–E-logits extraction always returns one of the 5 valid labels, so what the old name implied ("format-invalid") never occurs; the field is really "parsed, valid, non-E, WRONG". Analysis reports sample-level micro AND 57-task macro-average (macro is PRIMARY, matching the historical "Average of Tasks" convention) + four-domain (STEM/Humanities/Social Sciences/Other, `mmlu_category_map.py`, the public Hendrycks et al. 2021 grouping, verified against all 57 task names) + paired vs-baseline change with task-clustered bootstrap 95% CI (B=10000, resampling the 57 tasks) and an exact sample-level sign test. **Resume/completeness is task-level, not directory-existence**: a cell counts as complete only if all 57 task JSONs exist, each has the source's exact sample count, every sample carries full confident+unconfident answer/prob/softmax/logits fields, `run_meta_{size}.json` exists, and its recorded protocol/model/roles/tail_len/α/mask-sha256 exactly match the current invocation — a metadata mismatch is a FATAL refusal (never a silent resume), while a partial cell has only its missing tasks completed (complete tasks are never touched/overwritten); verified against 5 synthetic failure cases (missing file, sample-count mismatch, missing field, meta mismatch, full match) before use. **RR provenance note (do not write "RR was never tested")**: an RR-like condition WAS reported in an older results table, but the only locatable historical artifact (`RoleAnswer/llama3/mmlue/logits_mdf_4/*_20_11_20.json`) was found, on inspection, to carry **zero E options in any of its 57 rendered prompts**, a null `template` field, and implausible invalid rates up to 71% — i.e. it is plain 4-choice MMLU mislabeled as MMLU-E, not a protocol match — so RR is being RE-RUN from scratch here, not because no prior attempt existed but because none could be verified under this protocol. This provenance sentence is embedded in every `run_meta_{size}.json` this pipeline writes. **Launcher bug fixed 2026-09-08 (same trap as documented elsewhere in this file for other launchers): the launcher's positional-arg line used `${1:?usage: ...}` with a curly-braced choice list INSIDE the usage message itself — those braces terminated the `${...}` expansion at their own first `}`, so invoking with `baseline` set the shell variable to the literal string `baseline}` (trailing brace attached), which then failed argparse's `--condition` choices check. Fixed by removing braces from the usage text; `bash -n` does NOT catch this (it's a semantic bug, not a syntax error) — verified by invoking with real arguments and checking the resolved value, not just by re-running `bash -n`.** Both `get_answer_rr_cc_mmlue.py` and `analyze_rr_cc_mmlue.py` are `py_compile`-clean; the analysis script was additionally run end-to-end against a synthetic 9-cell × 57-task fixture (all output row counts matched exact expectations, no NaN/crash) since the real GPU run has not happened yet. Confidence mask + `cross_steering_confidence/` code must be manually `scp`'d to the server before launch — the assistant cannot run GPU jobs or reach the server. |
