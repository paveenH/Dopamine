...
| ZebraLogic-Easy 4-point exploration (`zebralogic-easy-v0`) | **PREPARED** — prereg `docs/PREREG_ZEBRALOGIC_EASY.md` frozen 2026-09-04 (commit `4f8e7ef`) before any cell. **NOT a transfer test** (α is NOT read from the frozen GSM8K record) and **NOT blind** — the public `grid_mode` test split's gold is literally `"___"` in all 1000 rows; real gold is in the **gated** `-private` repo needing per-account HF consent this environment lacks. Four doses per model can only support an observed argmax + near-optimal region — **never** a peak or inverted-U. Code isolated under `zebralogic/` |
| ProofWriter-OWA (`proofwriter-owa-v0`) | **⛔ SUSPENDED 2026-09-04 at the α=0 feasibility gate — do NOT run canary, pilot, or any α sweep.** Task-specific workpoint exploration under explicit CoT, **NOT** a GSM8K fixed-workpoint transfer test — no fixed α is read from the frozen record. Planned formal config: llama3 band `[11,20)` α∈`{−6,−4,0,+4}`, qwen2.5 band `[16,22)` α∈`{−6,0,+6,+8}`, batch 8, greedy, prefill-only tail=1, neutral, bare-string. `max_new_tokens` is FROZEN at 1024 (hard-rejected in code, not just documented) and is not being raised further. **Prompt is now `proofwriter-owa-cot-v1`** (≤5-step reasoning, no code/theory-restatement/step-repetition, terminal `Answer: True` / `Answer: False` / `Answer: Unknown` line) — this did **not** fix the gate: llama3 acc `.0333` parse_fail `.900` loop `.800` trunc `1.000`; qwen2.5 acc `.0333` parse_fail `.733` loop `.033` trunc `.033` (N=30 each; qwen2.5 got WORSE than its own `-v0` cell, whose parse_fail was `.400` — the added constraints appear to have hurt qwen2.5 rather than helped either model). Both models sit at ~1/30 correct. Continuing would compare format failure and loop degeneration, not steering. **Frozen wording: "the explicit-CoT ProofWriter OWA configuration has not passed feasibility preflight, so steering has not yet been tested" — NEVER "steering does not work on ProofWriter"** (no non-zero α has ever been run) and **NEVER "parseable-answer accuracy is unaffected"** (parse failure is scored fail-closed as incorrect, so it IS part of accuracy, not a side channel to it). The α=0-only evaluator's `No effective workpoint` verdict is a generic template message with **no statistical content** at a single dose and must not be cited as a finding. Results (both `-v0` and `-v1` preflight) are kept, not deleted or overwritten. **Resumption is now IN PROGRESS as a v2 probe (2026-09-05): a 1-shot feasibility test with ONE fixed `Unknown`-labeled train-split exemplar** (not the balanced 3-label probe originally planned — a single-label exemplar was chosen first and may carry an `Unknown`-label bias, which must be reported alongside any v2 result, not silently absorbed into it). Exemplar theory/question/label are the official train-split strings verbatim; a short reasoning sentence is hand-written from the official proof (or from theory+question if no proof exists) and explicitly labeled as this project's own construction, never official reasoning. Answer format also changes to a `#### True` / `#### False` / `#### Unknown` terminal line (strict, line-anchored only; loose/inline matches are diagnostic-only, last strict marker wins), `PROMPT_TEMPLATE_ID` → `proofwriter-owa-cot-v2`. A read-only lister (`proofwriter_owa/list_unknown_exemplar_candidates.py`) surfaces train-split `Unknown` candidates for manual selection; it does not pick or freeze anything itself. No v2 preflight may run until a human has picked and frozen one exemplar from that listing. If parse failure is still high after the v2 probe, the task is formally TERMINATED, not iterated further. Prereg `proofwriter_owa/PREREG_PROOFWRITER_OWA.md`. **Keep separate from ZebraLogic-Easy** (below) — different task, different prereg, different status (ZebraLogic is PREPARED/active; ProofWriter is SUSPENDED). |
...
```bash
# ZebraLogic (the ACTIVE line) and ProofWriter-OWA (SUSPENDED). Both subtrees
# are self-contained and BOTH REQUIRE python3.10: their tests import numpy
# through the runner, so a bare python3 dies with ModuleNotFoundError PARTWAY
# THROUGH, which reads as a broken suite rather than a wrong interpreter.
...
# subtree, ProofWriter's at the repo root beside every other run_*.sh.
bash -n zebralogic/run_zebralogic.sh && bash -n run_proofwriter_owa.sh

# ProofWriter's suspension is enforced IN THE LAUNCHER, not only in the docs,
# so it is a SEMANTIC guard that bash -n cannot see -- drive it with real
# arguments. All four blocked steps must exit 2; validate-data/preflight/
