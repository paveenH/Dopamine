...
6. Keep rationale concise. Detailed history belongs in the linked design/result document (`Note/Bandit.md`, `Note/Behaviour.md`, `Note/ReasoningBare.md`, amendment JSONs, `test_*.py`), not here.
...
| working on fixed-workpoint transfer (P3/P4/P4b/P4c) | the **FIXED-WORKPOINT TRANSFER SCOREBOARD** in **Active status** FIRST — it carries the five-row reading that no single section states — then that task's own section |
...
| working on the confidence-neuron line (MMLU-E / masks / means) | **`~/Documents/RSNResult/RoleHidden/`** — a SECOND off-repo workspace, also not in git, holding the six `analyze_*confidence*` scripts, `build_confidence_nmd_mask_for_steering.py`, and the mean/mask artifacts. It is **not** `RoleAnswer/`. Its line records `/opt/anaconda3/bin/python`, which on this box is a symlink to `python3.10` — the same interpreter `RoleAnswer/` uses, under a different name |
...
Evidence: this section's own history; Bandit.md §5 (BAI CLOSED).
...
Evidence: qwen2.5/cgt/seq_{asc,desc}_v5_qwen_full; Behaviour.md §3.3 CGT-Sequential 跨模型.
...
| Qwen GSM8K / MATH / CoT replication | **COMPLETE** 2026-08-21 — see `Note/ReasoningBare.md` §4 |
...
| CRUXEval-O Chat interface (`cruxeval-o-cot-chat-v1` + `-sweep-v1`) | **COMPLETE — NULL, llama3 ONLY** 2026-09-08 — three separate runs, none superseding another. (1) `{0,−6}` single comparison at llama3's own frozen workpoint (m=1, unadjusted): `−0.67 pp`, `p=.8804` — null, and it is NOT evidence "chat rescues llama" as an earlier reading of this one cell briefly implied before the four-point sweep. (2) Chat-CoT four-point sweep `{−6,−4,0,+4}` (Holm m=3, own family): all three non-zero doses non-significant, best `p_adj=.9667`(`−4`) — **NO chat-interface workpoint for CoT**. (3) Chat-No-CoT four-point sweep, same doses, own Holm m=3: also all non-significant, best `p_adj=.2939`(`+4`) — **NO chat-interface workpoint for No-CoT**. CoT and No-CoT are NEVER pooled into one Holm family. `+4` shortens generation sharply in both conditions (chars/tokens roughly halved) with no accuracy gain — output brevity is not reasoning improvement. **Frozen reading: fixing the bare-string interface (loop/truncation/no-marker all fell to low single digits) did NOT rescue llama3 CRUXEval-O steering** — the P4c null is a genuine model-specific effect, not a Bare-interface artifact. Do not read a healthy-format chat cell as "chat has no value" — it improved scorability/stability, not accuracy. Results: `Note/ReasoningBare.md` §6.1 Table 6.1, §6.2.5 Table 6.6, §6.4, §6.5 Table 6.10. Code: `get_answer_cruxeval_cot_chat.py` (single `{0,−6}` diagnostic), `get_answer_cruxeval_chat_sweep.py` + `eval_cruxeval_chat_sweep.py` + `run_cruxeval_chat_sweep.sh` (four-point CoT/No-CoT sweeps, own scorer imports `eval_cruxeval.py`'s primitives by reference rather than reimplementing them). `eval_cruxeval.py` gained an additive `--protocol` flag for the `{0,−6}` cell only; its default `cruxeval-p4c-v0` behaviour (Holm m=2 + neighbour/reverse diagnostics) is unchanged and was re-verified against fabricated fixtures after each edit. The sweep scorer hard-stops on cross-cell config drift (`mask_sha256`/`rendered_prompt_sha256`/`chat_template_hash`/`model_dir`/`max_new_tokens`/`batch_size`/`temperature`/`top_p`/`padding_side` must be byte-identical across all four cells of one sweep) and on a missing/extra alpha — neither check existed in the first draft and both were added after review. No Qwen; llama3 only throughout. Does not touch, redefine, or supersede the frozen P4c No-CoT/CoT fixed-workpoint results in the line above |
...
| CoT transfer follow-up (`cot-transfer-followup-v0`) | **COMPLETE — EXPLORATORY, post-hoc** 2026-09-03 — designed AFTER seeing the P4/P4b/P4c No-CoT nulls, same fixed workpoints, α never re-selected; **NOT a confirmatory replication or blind validation**. Own Holm family **m=6** (withheld unless all 6 (task,model) pairs present). Three cells are accuracy-positive and Holm-passing (BBH×Llama, BBH×Qwen, CRUXEval-O×Qwen); LogiQA stays null on both. `ec` falls more sharply under CoT in those three, but that **co-occurs with** the gain and is post-treatment — never "causally mediates". BBH's CoT α=0 does not beat its No-CoT α=0, so the gain is the `CoT × steering` interaction, not CoT alone. Protocol `docs/PREREG_COT_TRANSFER_FOLLOWUP.md`; results `Note/ReasoningBare.md` §6.3 |
| ZebraLogic-Easy 4-point exploration (`zebralogic-easy-v0`) | **COMPLETE + CLOSED, 2026-09-05.** Protocol `zebralogic-easy-v0` + `zebralogic-easy-amend-01` (`docs/PREREG_ZEBRALOGIC_EASY.md` frozen 2026-09-04 commit `4f8e7ef`, amendment `docs/zebralogic_easy_amendment_01.json`). **Task-specific four-point exploration, NOT a GSM8K fixed-workpoint transfer test** — α is not read from the frozen GSM8K record. Easy subset = 280 items (`2*2/2*3/2*4/2*5/2*6/3*2/3*3`, 40 each). Fixed budget `max_new_tokens=1024`, batch_size 8, greedy, bare-string, prefill-only, tail=1. **Main scoring = FIRST complete top-level JSON with a non-empty `solution`** (amend-01, replacing the official LAST-complete-JSON convention); a missing/incomplete answer scores incorrect and stays in the overall denominator; the official LAST-answer scorer is kept only as a `*_last` sensitivity field — **FIRST and LAST are different estimands (different submission time points), never call one "the real accuracy" and the other biased**. Private gold (`allenai/ZebraLogicBench-private`, gated, 1000-row private split) was confirmed reachable from the server (`CHECK_ACCESS passed`) before the formal sweep; both models' formal four-dose sweeps ran N=280, each model's own four α on one GPU/one machine (`cross_gpu_within_machine: false`), Holm family complete (m=3 per model). **Llama3.1-8B `[11,20)` α∈{−6,−4,0,+4}**: α=0 puzzle_acc 36.79% (highest observed point, NOT an established workpoint); −6 35.71% (Δ−1.07pp, p_adj=.749); −4 33.93% (Δ−2.86pp, p_adj=.512); +4 32.14% (Δ−4.64pp, p_adj=.198); no non-zero α is a significant improvement; no_answer_rate flat 24.3–25.4% across all four α (not a steering-driven trend); truncated_rate=1.000 and strict_loop_rate≈90–95% in every cell. **Qwen2.5-7B `[16,22)` α∈{−6,0,+6,+8}**: α=0 puzzle_acc 34.64%; −6 30.00% (Δ−4.64pp, p_adj=.158); +6 36.07% (Δ+1.43pp, p_adj=.678, numerically highest but NOT an established workpoint); **+8 23.93% (Δ−10.71pp, raw p=1.3e−4, Holm p_adj=4.0e−4) is a significant DEGRADATION** — an over-steering boundary where no_answer_rate jumps 0%→60.7%. Qwen's non-crashed cells (α=0/−6/+6) show a large FIRST-vs-LAST divergence (e.g. α=0 FIRST 34.64% vs LAST 15.36%), indicating the first-committed grid is frequently revised later in generation — this is reported as a FIRST/LAST estimand difference, not as "LAST underestimates true accuracy." `revision_right_to_wrong_sum`/`revision_wrong_to_right_sum` are **cell-level (grid-cell) counts, not puzzle counts** — do not describe them as puzzle-equivalent. Llama's aggregate FIRST≈LAST does NOT imply per-item agreement (opposing per-item revisions can cancel in the aggregate). Harder sizes (`2*6`, `3*3`) show elevated no_answer_rate at every α — report this only as "concentrated in harder sizes," never attributed to difficulty alone, since it is confounded with looping and the fixed 1024-token budget. **Frozen verdict: "NO non-zero alpha cleared Holm as a SIGNIFICANT IMPROVEMENT vs alpha=0. No effective ZebraLogic-Easy workpoint was detected among the four sampled points."** Per the user's decision the line stops here — no additional doses, no token-budget increase, no further prompt/scorer changes, no follow-up; all code, the amendment, and every result artifact are kept. Code isolated under `zebralogic/` |
| ProofWriter-OWA (`proofwriter-owa-v0`) | **✅ COMPLETE + CLOSED / TERMINATED, 2026-09-05.** Task-specific dose exploration under explicit CoT, **NOT** a GSM8K fixed-workpoint transfer test — no fixed α is read from the frozen record, and its result is not part of the fixed-workpoint transfer scoreboard. Protocol stays `proofwriter-owa-v0`; prompt is the v2 revision `proofwriter-owa-cot-v2` — 1-shot, ONE fixed official train-split `Unknown` exemplar (theory/question/label verbatim from the train split; the accompanying reasoning sentence is this project's OWN construction, not the official proof, and may carry an `Unknown`-label bias that travels with every v2 result). Main scoring takes the **FIRST** strict, line-anchored `#### True|False|Unknown` marker (revised 2026-09-05 from an earlier last-marker convention — first commitment should not be overwritten by a degenerate trailing loop); zero strict markers scores **incorrect** and stays in the overall-accuracy denominator (`no_answer_rate` reported separately); the last-marker accuracy is kept only as a `sensitivity_last_answer_accuracy` diagnostic. Formal config: llama3 band `[11,20)` α∈`{−6,−4,0,+4}`, qwen2.5 band `[16,22)` α∈`{−6,0,+6,+8}`, `max_new_tokens=1024` (frozen, hard-rejected in code at any other value), batch 8, greedy, bare-string, prefill-only tail=1, neutral. **Result (`proofwriter_owa/results/formal_sweep_v2.json`, N=300/cell): Llama's `α=+4` is Holm-significant** — accuracy `.1033→.2167` (Δ=+11.33 pp, p_adj=2.27e−4) — **but `no_answer_rate` falls in step (.793→.603) while truncation stays at a constant 1.000 and loop_rate sits at .900–.950 across all four llama cells, so this is read cautiously as steering raising the rate of valid answer submission, NOT as demonstrated reasoning improvement.** Qwen's highest observed point is `+8` (accuracy `.4633→.5200`, Δ=+5.67 pp) but does **not** survive Holm (p_adj=.257) — no effective workpoint detected. Per the user's decision, the line stops here: no further doses, no prompt revision, no token-budget increase, no ProofWriter follow-up. All prior code and every v0/v1/v2 result artifact are kept, not deleted. See `Note/ReasoningBare.md` §5.5c for the full table. **Keep separate from ZebraLogic-Easy** (below) — different task, different prereg, different protocol and results; both are now COMPLETE + CLOSED but must not be conflated. |
| ProofWriter-OWA chat-template interface diagnostic (`proofwriter-owa-chat-v1`) | **COMPLETE, 2026-09-08 — an INDEPENDENT interface-correction experiment layered on the CLOSED `proofwriter-owa-v0` line above, NOT a reopening of it and NOT part of the fixed-workpoint transfer scoreboard.** Motivation: the closed bare-string line showed llama's `α=+4` gain riding partly on `no_answer_rate` falling (.793→.603) with truncation pinned at 1.000 and loop_rate ≈.90–.95 in every cell — i.e. the bare-string interface itself might be manufacturing the failure mode steering appears to fix. A 30-item llama-only α=0 pilot (`proofwriter_owa/diag_chat_template.py`, output `chat_diag_eval_llama3_v2.json`) held prompt body/exemplar/marker convention/gold/parser identical and changed ONLY the wrapping — bare-string vs `tokenizer.apply_chat_template(..., add_generation_prompt=True)` — and found chat wrapping sharply cuts no-answer/loop/truncation, confirming the bare-string interface (not reasoning capacity) as the dominant driver of llama's degenerate generations. **Formal 4-point chat sweeps then ran on both models**, same 300-item manifest/order/gold/exemplar/mask/band/α-set/generation params as each model's own frozen bare formal sweep, with chat wrapping as the sole added variable: llama3 `−6/−4/0/+4` band `[11,20)` → `proofwriter_owa/results/formal_sweep_chat_v1.json`; qwen2.5 `−6/0/+6/+8` band `[16,22)` → `proofwriter_owa/results/formal_sweep_chat_v1_qwen25.json`. Own Holm **m=3 per model, vs that model's own chat α=0** — never compared for significance against the bare-string cells (descriptive contrast only). **Result: llama3 has NO Holm-significant chat workpoint** (best point `α=−6` +6.33pp, `p_adj=.244`) — the bare sweep's significant `+4` gain does NOT reproduce once the no-answer-rate confound is removed by chat wrapping, consistent with that gain being substantially a submission-rate artifact of the bare interface. **Qwen2.5 DOES show a Holm-significant chat workpoint at `α=+8`** (+6.67pp, `p_adj=.0303`), matching its bare-sweep dose ordering. **Qwen2.5 `α=−6` under chat produces a CATASTROPHIC, highly significant collapse** (accuracy `.4100→.0067`, `p_adj=2.26e−36`, `no_answer_rate=.990`) that has no counterpart in the bare sweep (bare `α=−6` was Qwen's single best bare point, `.4933`). Inspection of the raw generations (297/300 samples) shows the model emitting a single bare label token (e.g. `'False'`, 2 tokens total) with **no CoT text and no `#### ` marker at all**, terminating via natural EOS — categorically different from truncation or looping, and confirmed NOT a prompt/steering-configuration bug (`bare_prompt_sha256` matches the bare sweep exactly; `steering_fires=1800=L×300` as expected). **Frozen reading: this shows "workpoint" is interface-dependent** — a model's best bare-string steering direction can become its worst chat-interface direction, and the collapse is a wholesale abandonment of the output-format instruction, not a reasoning-quality degradation. Code: `proofwriter_owa/get_answer_proofwriter_owa_chat.py` (parameterized `--model {llama3,qwen2.5}`, single `MODEL_CONFIG` dict, one shared `CHAT_PROTOCOL="proofwriter-owa-chat-v1"` string for both models — model identity is carried by `meta.prompt_wrapper_id` [`llama3-chat-template-v1` / `qwen2.5-chat-template-v1`], NOT by forking the protocol string per model), `run_proofwriter_owa_chat_v1.sh` (`sweep`/`eval` stages, second positional arg selects model, each model's `CELL_FILES` array built entirely inside its own `case` branch with zero shared path variables between branches). `eval_proofwriter_owa.py` gained an additive `--protocol` CLI flag (default unchanged, `proofwriter-owa-v0`) and `CONSISTENCY_FIELDS` gained `prompt_wrapper_id`/`chat_template_applied`/`chat_template_hash` — no parser, marker family, or bare-sweep scoring logic touched. Does not touch, redefine, or supersede the CLOSED bare-string `proofwriter-owa-v0` result above. `Note/ReasoningBare.md` NOT modified as part of this line, per explicit instruction. |
| FinQA (`finqa-v0`) | **COMPLETE + CLOSED, 2026-09-07.** Task-specific dose exploration on FinQA, **NOT** a GSM8K fixed-workpoint transfer test — α swept fresh on this task, not read from the frozen GSM8K record, and not part of the fixed-workpoint transfer scoreboard. Direct-numeric-answer setting (no official program/DSL required), explicit CoT, report text + linearized table + question as input, final marker `#### <numeric answer>`, MAIN = first legal marker (no legal marker scores incorrect); `last_acc`/`answer_rate`/`answered_acc`/loop/truncation/gen-length kept as diagnostics only. **Scoring is a CUSTOM direct-numeric-answer evaluator (marker parse + FinQA-style normalization: $/commas/%/units + relative tolerance), NOT the official FinQA program/DSL execution-based scorer — these numbers are NOT comparable to official FinQA leaderboard accuracy.** 300-item stratified sample (program-op-count × table-size), llama3 band `[11,20)` α∈`{−6,−4,0,+4}`, qwen2.5 band `[16,22)` α∈`{−6,0,+6,+8}`, `max_new_tokens=512`, bs=8, greedy, bare-string, prefill-only tail=1, neutral, Holm m=3 per model. **Result (`docs/finqa_formal_evaluation.json`, N=300/cell): NO effective positive workpoint detected on either model.** Llama3: α=0 first_acc **14.33%**; `−6` (8.67%) and `−4` (9.00%) are Holm-significant **decreases**; `+4` (15.33%, Δ=+1.00pp) is not significant. Qwen2.5: α=0 first_acc **20.67%**; `−6` (10.00%) is a Holm-significant decrease; `+6` (20.67%) is unchanged; `+8` (25.67%, Δ=+5.00pp) is the largest positive point but does **not** survive Holm (p_adj=.154) — a positive TREND only, not an effective workpoint. **Interpretive boundary, both models**: Llama's generations show pervasive post-answer loop/truncation (93–97% loop, 100% truncated in every cell) — the MAIN metric (first-answer) is unaffected by this and is the number reported above, but it means Llama's raw generations are not natural-length text. Per the user's decision the line stops here: no further doses, no prompt/parser/token-budget changes, no FinQA follow-up. All code (`finqa/`) and result artifacts are kept, not deleted. |
| GSM-Symbolic (`gsm_symbolic-v0`) | **COMPLETE, 2026-09-08.** Task-specific dose exploration on the official `apple/GSM-Symbolic` dataset (`main`/`p1`/`p2` configs), NOT an independent cross-domain transfer test — a same-family numeric/template-perturbation robustness check on GSM8K's own frozen workpoints (llama `−6`, qwen `+8`), read from the frozen record and not re-searched; results are `Note/ReasoningBare.md` §6.1/§6.2.1/§6.4/§6.5 (Tables 6.1/6.2/6.9/6.8). 300 cluster-balanced items/config (900/model), unique key `sample_id="{config}:{original_id}:{instance}"` (HF's own `id` field equals `original_id`, a CLUSTER id shared by every instance of one original problem — using `id` alone as the row key silently collapses 300 rows to 100 (main/p1) or 50 (p2) per cell, corrupting every downstream statistic; `gsm_symbolic/data_gsm_symbolic.py::assert_unique_sample_ids` now hard-stops on this at generation time, and `gsm_symbolic/fix_sample_id_bug.py` is the one-time offline repair for cells generated before the fix — both the No-CoT and CoT trees needed it, applied with `.bak` backups, verified row-count-invariant and non-key-field-identical). Reuses the frozen GSM8K explicit-CoT/No-CoT prompt pair (`select_templates_gsm8k(suite="default", cot=True/False, wording="plain")`, No-CoT literally drops only the "Let's think step by step." line) and the frozen `extract_gsm8k_answer`/`is_correct_gsm8k` scorer unmodified; No-CoT and CoT are separate output trees (`gsm_symbolic/` vs `gsm_symbolic_cot/`) that must never be conflated. **PRIMARY inference is a config-equal-weighted (`main`/`p1`/`p2` weighted equally, never by row count) paired cluster bootstrap resampling `original_id` clusters, Holm `m=3` per model per CoT condition** — row-pooled exact McNemar is SENSITIVITY ONLY and is not shown in the frozen tables. **Result: No-CoT — llama `−6` (+9.44 pp, `p_adj=.0003`) AND `−4` (+4.22 pp, `p_adj=.0164`) both positive, `+4` a significant degradation (−9.22 pp); qwen `+6` (+7.33 pp) AND `+8` (+13.56 pp) both positive, `−6` not significant. CoT — qwen `−6/+6/+8` all positive (+3.56/+12.22/+12.11 pp); llama has NO positive significant result at all (`−6/−4` not significant, `+4` a significant −15.33 pp degradation).** An EXPLORATORY commitment/answer-formation panel (`gsm_symbolic/commit_panel_gsm_symbolic.py`, imports the frozen `early_candidate_detector.has_early_candidate` and `p3/precandidate_reasoning.per_sample` verbatim, never reimplements them; cross-checks recomputed accuracy against each cell's stored meta accuracy on all 48 cells before trusting any commitment number) shows every significant-positive dose comparison co-occurring with `early_candidate_rate` falling and `reason_first_rate` rising — Qwen's swings are large (e.g. No-CoT `+8`: `ec` .9622→.0656, `reason_first` 0→.9844) — **but Llama CoT `−6` is the load-bearing counterexample**: commitment timing moves the SAME direction (`ec` .2833→.1644, `reason_first` .5926→.7929) while accuracy does NOT improve significantly (+1.11 pp, `p_adj=.527`). Frozen wording: **"commitment change co-occurs with effective steering but is not a sufficient condition for accuracy improvement"** — consistent-with evidence, never causal mediation. `posN` (normalized position of the first parseable `####` marker) is AUXILIARY only, anchored to a formatting event rather than the answer-candidate value, and does not need to move the same direction as accuracy. `no_answer_rate=0` in every cell only means the frozen fallback chain always extracts SOME comparable value — it does NOT mean every generation produced a well-formed `####` submission; `no_marker_rate`/`marker_unparsed_rate` are the separate, non-zero readouts for that. Results: `/tmp/eval_nocot_llama3_FINAL.json`, `/tmp/eval_nocot_qwen25_FINAL.json` (post-repair formal No-CoT stats), `/tmp/gsm_symbolic_commit_panel.json` (commitment panel, exploratory, outside the accuracy Holm family). Code isolated under `gsm_symbolic/`, no existing GSM8K/MATH/other-task runner touched. |
| MMLU-E explicit-confidence neurons (`mmlue-confidence-hs-v0`) | **COLLECTION + OFFLINE ANALYSIS COMPLETE 2026-09-08; causal steering results are tracked in the next line.** Llama3.1-8B-Instruct, all 57 `detection/task_list.py` MMLU tasks, roles `confident`/`unconfident` (the ACLARR §app:prompt-template-confidence explicit-confidence anchors, via the existing `templates["neg"]` / `utils.construct_prompt` code path that already special-cases any role containing `"confident"`), MMLU-E (`suite=default --use_E`, A–E, E="I am not sure"), bare-string, no CoT. **Collection** (`run_hidden_mmlue_confidence_hs.sh`, fail-closed 3-preflight launcher, `verify_mmlue_confidence_hs.py` acceptance) is unchanged from the prior entry — real paths: MMLU JSON `${BASE_DIR}/mmlu`, HS `${BASE_DIR}/hidden_states/llama3/mmlue_confidence/`, answers manually moved to `${BASE_DIR}/answer/llama3_confidence/`. **`mean/mean_diff_confidence.py`'s divergent-pairs-only draft was SUPERSEDED by `mean/mean_diff_confidence_local.py`** (server-side, two-pass fail-closed design: Pass 1 validates every one of the 57 tasks — file presence, paired H5 shape equality, H5 N == answer count, exact `(N,33,4096)` shape — BEFORE any accumulation; any single task failure aborts with exit 1 and writes NOTHING) — **the MAIN result uses ALL PAIRED samples (14,042 across 57 tasks), NOT divergent-only**, per explicit correction; divergent-only is a separate `sensitivity_divergent_only/` output. Both float32 (production, cast to fp16) and float64 (audit) accumulators run in parallel; float32 vs float64 consistency error was 5.96e-8 vs 5.55e-17 (9 orders of magnitude apart, confirming the float32 gap is pure summation-order rounding, not a logic bug). Outputs `confident_mean_8B.npy` / `unconfident_mean_8B.npy` / `confidence_diff_8B.npy` (all `(1,1,33,4096)` fp16, matching the legacy `diff_mean_{size}.npy` convention) synced to `~/Documents/RSNResult/RoleHidden/llama3_confidence/`. **Both `verify_mmlue_confidence_hs.py` and `mean_diff_confidence_local.py` run on the SERVER with `python`, NOT `python3.10`** (server-interpreter correction applied to both scripts' docstrings and to this file). |
| ↳ Direction / overlap / contribution / top-k analysis (local, `RoleHidden/`) | **COMPLETE 2026-09-08**, all `/opt/anaconda3/bin/python`, none modify any existing script/mean/mask. `role_direction = expert_mean − nonexpert_mean`, `confidence_direction = confident_mean − unconfident_mean` (sign never auto-flipped). **`analyze_role_confidence_direction.py`**: injection-band (decoder 11–19) concatenated cosine = **0.6063** (Pearson 0.6063, nearly identical — cosine plotted dashed to stay visible under the Pearson line), per-layer cosine mean/median/min/max = 0.499/0.475/0.158/0.764; `llama3_8B_diff.npy` explicitly excluded (stale, doesn't match current mean pair), `llama3_logits_8B_diff.npy` used only as a provenance cross-check (max abs error 4.88e-4, non-gating). **`analyze_confident_unconfident_divergence.py`**: Pearson(confident_mean[l], unconfident_mean[l]) falls from 0.989 (pre-band) to 0.878 (band mean), minimum at L19 (r=0.819); confidence-direction L2 norm peaks in-band at L19 (~6.4), globally at L32 (52.96). **`analyze_confidence_nmd_mask.py`**: built the Confidence NMD mask via `importlib`-imported `detection/nmd.py::get_nmd_mask` (never reimplemented from memory) — same formula confirmed by direct code read: `diff = confident_raw − unconfident_raw` (float16, no upcast), per-layer top-k-by-absolute-value selection (`argsort` on the abs diff, last `top_k` indices), `top_k=int(4096×0.5/100)=20`, **stores the SIGNED diff value, not a binary mask**. Result: `(32,4096)` fp16, 180 total nonzero (20/layer, L11–19), **format-identical to the Role mask** (shape/dtype/nonzero-per-layer all match) — confirms NMD needs ONLY the two mean files, never sample-level HS/variance, so no server sync was needed to unblock this. **`analyze_role_confidence_overlap.py`**: Role/Confidence top-20 support overlap is far above random at every layer except L12 (random expected ≈0.098/layer; observed intersection 0–10, hypergeometric p<0.005 in 8/9 layers), peaking at **L18 (intersection=10/20, Jaccard=0.333)**; overlap neurons' sign agreement is ≈1.0 from L14 on. **`analyze_role_confidence_contribution.py`**: linear decomposition of the L11–19 concatenated cosine (0.606298, reproduced exactly) into shared_top / role_only / confidence_only / neither_top — **neither_top (36,550 of 36,864 positions) carries 86.3% of the signed dot product and 88.6% of absolute alignment mass**; shared_top (only 46 positions) is per-neuron more efficient (8.0% of dot from 0.13% of neurons) but its ABSOLUTE contribution is small — **dense alignment is NOT primarily carried by shared-top neurons**, it is a broad, diffuse sum over thousands of non-top positions. **`analyze_role_confidence_topk_sensitivity.py`**: overlap is stable across k∈{5,10,20,40,80,160} (overlap rate 0.20–0.33 throughout, enrichment 5.5×–237× vs random, reproduces the k=20 mask-based numbers exactly); cumulative absolute alignment mass needs 617/2557/7249/14182 of 36,864 positions to reach 25%/50%/75%/90% of the alignment mass — confirms "small stable high-density core (top-k robust, high sign-agreement) + large diffuse background" rather than a small-core-dominates picture. |
| ↳ RR/CC causal steering on MMLU-E (`mmlue-confidence-steering-v0`) | **CC RUN COMPLETE with results (small-dose supplement complete 2026-09-10); RR NOT RUN under this protocol (see provenance note below — a separate, differently-scoped RSN unconfident run exists and is reported alongside, not as a substitute).** Question: does the NEW Confidence NMD mask (`confidence_0.5_11_20_8B.npy`, built by `RoleHidden/build_confidence_nmd_mask_for_steering.py` — **that builder lives in the offline `RoleHidden/` workspace, NOT in this git repo (verified 2026-09-14), like the six `analyze_*` scripts in the line above**; it does not re-derive the NMD formula but imports `detection/nmd.py::get_nmd_mask` by file path and asserts its output is numerically identical to the earlier `llama3_confidence/neuron_selection/confidence_top_neuron_mask_8B.npy` before writing the correctly-named copy, so its only job is the filename convention the steering loader requires; byte-identical to the mask from that line, sha256 `f9d8eed8b4155e8d9febad5f95ec7adfc3e36a9d06fc585adb54be2dcbbfbe7c` recorded in its manifest) have a causal effect on confident/unconfident behavior on MMLU-E? **Deliberately simplified from an earlier RC/CR/random-support cross-steering design** (kept, unused, under `cross_steering_confidence/{get_answer_cross_steering_mmlue.py,run_cross_steering_mmlue.sh,analyze_cross_steering_mmlue.py,build_cross_steering_vectors.py}` for a later stage) — **this stage runs ONLY CC, the mask used AS-IS with no direction/support decomposition and no norm-matching**. Code: `cross_steering_confidence/get_answer_rr_cc_mmlue.py` (standalone, does not import or modify `get_answer_regenerate_logits.py` — that script can only load a mask-named file × integer α, so a new script was needed to load either of exactly two named masks), `cross_steering_confidence/run_rr_cc_mmlue.sh` (`CC_SMALL` adds only α=+0.5/+1 and does not touch the completed +2/+4 cells; RR is explicitly REFUSED at the launcher level per the provenance note below), `analyze_rr_cc_mmlue.py`. **Actual formal dose set collected: α∈{0, +0.5, +1, +2, +4}**; +6 was never run, and the earlier −2/−4 arm is excluded from the formal positive-dose summary. The protocol uses `tail_len=1`, decoder layers 11–19, all 57 tasks and both `confident`/`unconfident` prompt roles; α=0 baseline runs once. **`invalid` was renamed `wrong_non_E`** — the argmax-over-A–E-logits extraction always returns one of the 5 valid labels, so what the old name implied ("format-invalid") never occurs; the field is really "parsed, valid, non-E, WRONG". Analysis reports sample-level micro AND 57-task macro-average (macro is PRIMARY, matching the historical "Average of Tasks" convention) + four-domain (STEM/Humanities/Social Sciences/Other, `cross_steering_confidence/mmlu_category_map.py`, the public Hendrycks et al. 2021 grouping, verified against all 57 task names) + paired vs-baseline change with task-clustered bootstrap 95% CI (B=10000, resampling the 57 tasks) and an exact sample-level sign test. Pooled conditional accuracy is `accuracy/(1-E-rate)` over all 14,042 paired samples; its primary CI and paired-difference CI use sample-level bootstrap (B=10000), while task-macro conditional accuracy remains supplementary because tasks with 100% E have undefined values and would otherwise change the effective task set across doses. Raw results live under `RoleAnswer/llama3/mmlue_confidence/{baseline,CC}/`; the primary exports are `analysis/summary_task_macro.csv`, `analysis/summary_pooled_conditional_accuracy.csv`, and `analysis/paired_vs_baseline.csv`; the pre-small-dose snapshot remains under `analysis_before_small_dose/`. **Resume/completeness is task-level, not directory-existence**: a cell counts as complete only if all 57 task JSONs exist, each has the source's exact sample count, every sample carries full confident+unconfident answer/prob/softmax/logits fields, `run_meta_{size}.json` exists, and its recorded protocol/model/roles/tail_len/α/mask-sha256 exactly match the current invocation — a metadata mismatch is a FATAL refusal (never a silent resume), while a partial cell has only its missing tasks completed (complete tasks are never touched/overwritten); verified against 5 synthetic failure cases (missing file, sample-count mismatch, missing field, meta mismatch, full match) before use. **RR provenance note, and this is now a FROZEN launcher-level decision, not just a documentation caveat**: an RR-like condition WAS reported in an older results table, but the only locatable historical artifact (`RoleAnswer/llama3/mmlue/logits_mdf_4/*_20_11_20.json`) was found, on inspection, to carry **zero E options in any of its 57 rendered prompts**, a null `template` field, implausible invalid rates up to 71%, and **no α=+6 cell at all** — i.e. it is plain 4-choice MMLU mislabeled as MMLU-E, not a protocol match. When later asked to reuse "existing matching-protocol RR 0/2/4/6 data", this was re-verified (no α=+6 RR cell exists anywhere, the same historical candidate remains disqualified) and reported back rather than silently complied with or silently overridden; the user then confirmed skipping RR entirely for this round. `cross_steering_confidence/run_rr_cc_mmlue.sh` now **actively refuses** `RR` as a condition with an explanatory message (rather than silently no-op'ing or running it), and this refusal is itself part of the frozen launcher, not a TODO. **Launcher bug fixed 2026-09-08 (same trap as documented elsewhere in this file for other launchers): the launcher's positional-arg line used `${1:?usage: ...}` with a curly-braced choice list INSIDE the usage message itself — those braces terminated the `${...}` expansion at their own first `}`, so invoking with `baseline` set the shell variable to the literal string `baseline}` (trailing brace attached), which then failed argparse's `--condition` choices check. Fixed by removing braces from the usage text; `bash -n` does NOT catch this (it's a semantic bug, not a syntax error) — verified by invoking with real arguments and checking the resolved value, not just by re-running `bash -n`.** Both `get_answer_rr_cc_mmlue.py` and `analyze_rr_cc_mmlue.py` are `py_compile`-clean and were validated against a synthetic fixture before the real run. **RESULT (macro, 57-task average, `analyze_rr_cc_mmlue.py` output under `RoleAnswer/llama3/mmlue_confidence/analysis/`):**

| Steering | Prompt | alpha | Accuracy | Delta Accuracy | E-ratio | Delta E-ratio |
|---|---|---|---|---|---|---|
| Baseline | confident | 0 | 65.20% | -- | 2.31% | -- |
| Baseline | unconfident | 0 | 0.32% | -- | 99.39% | -- |
| CC | confident | +0.5 | 66.62% | +1.42 pp | 0.75% | -1.56 pp |
| CC | confident | +1 | 66.68% | +1.48 pp | 0.29% | -2.02 pp |
| CC | confident | +2 | 56.99% | -8.20 pp | 1.51% | -0.80 pp |
| CC | confident | +4 | 32.39% | -32.80 pp | 0.02% | -2.29 pp |
| CC | unconfident | +0.5 | 2.57% | +2.25 pp | 96.08% | -3.31 pp |
| CC | unconfident | +1 | 12.13% | +11.81 pp | 81.58% | -17.81 pp |
| CC | unconfident | +2 | 53.62% | +53.30 pp | 9.04% | -90.35 pp |
| CC | unconfident | +4 | 27.42% | +27.10 pp | 0.02% | -99.38 pp |

  **Conditional-accuracy result (pooled, 95% CI; paired difference vs α=0 in parentheses):** confident α=0 `66.56% [65.78,67.33]`; +0.5 `67.19% [66.40,67.95]` (`+0.63 pp [0.29,0.98]`); +1 `67.09% [66.31,67.87]` (`+0.53 pp [0.03,1.02]`); +2 `56.31% [55.50,57.13]` (`-10.25 pp [-11.11,-9.41]`); +4 `31.83% [31.07,32.61]` (`-34.73 pp [-35.80,-33.64]`). Unconfident α=0 `49.46% [39.21,59.60]`; +0.5 `60.35% [56.60,64.12]` (`+10.89 pp [1.60,20.22]`); +1 `60.22% [58.54,61.91]` (`+10.76 pp [0.97,21.14]`); +2 `58.24% [57.39,59.07]` (`+8.78 pp [-1.42,19.16]`); +4 `27.36% [26.61,28.09]` (`-22.10 pp [-32.41,-11.99]`). **Interpretation boundary:** conditional accuracy conditions on a dose-dependent event (not choosing E), and the α=0 unconfident non-E subset is tiny. Its movement shows that abstention and correctness among answered items do not change mechanically one-for-one, but it does NOT by itself identify restored knowledge or hold the answered subset fixed.

  **Reading, both prompt directions must be reported together, never one alone**: +0.5/+1 are the lowest-cost sampled doses — confident accuracy/conditional accuracy hold or rise slightly while E-rate falls. Positive-α CC steering also suppresses E-option abstention on the `unconfident` prompt, already measurably by +1 (99.39%→81.58%) and almost completely by +2/+4. However, the large high-dose unconfident accuracy increases cannot be cited alone as restored knowledge: `wrong_non_E` rises as abstention collapses, confident accuracy falls sharply from +2 onward, and +4 drives both prompts into a degraded regime. **α=+4 is over-steering; α=+0.5/+1 are the usable low-dose region among sampled points.**

  **A separate RSN (Role-mask, `nmd_0.5_11_20_8B.npy`) `unconfident`-prompt steering result exists from a DIFFERENT run** (not part of this CC pipeline, not sharing this pipeline's baseline, α set, or `run_meta` provenance chain): baseline unconfident 0.34% acc / 99.42% E-ratio; α=+4 → 11.36% acc / 78.87% E-ratio (Δ +11.02 pp / −20.55 pp); α=+5 → 20.60% / 58.00% (Δ +20.26 pp / −41.42 pp); α=+10 → 40.31% / 0.08% (Δ +39.97 pp / −99.34 pp). **Its baseline numbers differ slightly from the CC pipeline's own baseline** (0.34/99.42 vs 0.32/99.39) — a different batch, not a shared/reusable baseline — so RSN and CC rows must each be read against their own baseline, and RSN vs CC magnitude at matched α is NOT a strength comparison (masks are not norm-matched; RSN needs α≈+10 to reach an E-suppression level CC reaches by α=+4, but this reflects each mask's own per-layer norm, not "CC is stronger"). This RSN row does **not** substitute for, or get merged into, the RR condition this pipeline's launcher refuses — RR (Role mask under THIS protocol's exact prompt/task/sampling pipeline) still has no verified MMLU-E result. Confidence mask + `cross_steering_confidence/` code were manually `scp`'d to the server before launch — the assistant did not and cannot run GPU jobs itself. |
| GSM8K Confidence-neuron steering pilot (`gsm8k-confidence-pilot-v0`) | **RUN, 5 CELLS COMPLETE, 2026-09-09 — descriptive pilot, not Holm-tested.** Question: does swapping the RSN NMD mask for the Confidence NMD mask (`confidence_0.5_11_20_8B.npy`, same mask family as the MMLU-E confidence-neuron line above) also move GSM8K accuracy and commitment dynamics — **ONLY** the mask changes; model/prompt/template/`<|eot_id|>`/greedy/injection band (decoder 11–19, prefill-only tail=1)/`max_new_tokens=768`/`batch_size=24`/plain wording/neutral role are byte-identical to the authoritative `run_gsm8k.sh` (MACHINE=182) No-CoT protocol. **New, independent launcher `run_gsm8k_confidence.sh`** (no edits to `get_answer_regenerate_gsm8k.py`, `run_gsm8k.sh`, the RSN mask, or any RSN output) — the script only needed `--mask_type confidence` (the generic mask-name template `{mask_type}_{percentage}_{start}_{end}_{size}.npy` already resolves the confidence file with no code change). Doses α∈{−4,−2,0,+2,+4} (pilot; narrower than RSN's −8…+8), role=neutral only. α=0 was **re-run in this experiment's own tree** rather than copied from the RSN `mdf_0` JSON — protocol-identical (mask×0 is all-zero regardless of mask file) but kept as its own complete evidence chain per the user's choice. Output: `components/llama3/answer_mdf_gsm8k_confidence/mdf_{alpha}/` (parallel to, independent of, `answer_mdf_gsm8k[_cot]`). **Analysis reuses `analyze_cot_metrics.py` / `analyze_loop_anxiety.py` / `analyze_first_last_acc.py` BY IMPORT ONLY** — two new local scripts in `RoleAnswer/` (`analyze_confidence_gsm8k.py`, `analyze_confidence_cot_metrics.py`) call `analyze_cot_metrics.compute_cell(fp, TASK_CFG["gsm8k"])` and `analyze_loop_anxiety.analyze_file(mode="oscillation"|"anxious_repeat")` directly, pointed at the confidence tree with a 5-α list — `analyze_cot_metrics.py`'s own `--table dose` CLI could NOT be reused as-is because `DOSE_DIRS["gsm8k"]` hardcodes the 9 RSN directory names under `llama3/gsm8k/`, not the 5-dose confidence tree; neither existing script's source was modified. Confirmed **`preempt_any` (the RSN table's "Premature, either rule" column) is the pre-`earlycand-v1` detector and is NOT the same definition as the later frozen `early_candidate_detector.py`** — the pilot table below uses `preempt_any` throughout, matching the RSN §2.1 table's own column, never mixed with `earlycand-v1`. **§2.1-style five-α side-by-side (RSN / CSN, same five doses only — RSN's own ±6/±8 endpoints are out of scope for this pilot and are NOT extrapolated to)**: Accuracy RSN 73.0/69.0/60.0/57.0/55.3% vs CSN 51.0/60.3/60.3/55.7/46.3% at α=−4/−2/0/+2/+4 — RSN is monotone-decreasing over this range, CSN is an inverted-U peaking at 0/−2; Median `####` position RSN 25/18/18/19/16% vs CSN 20/20/18/17/18% — RSN trends down with α here, CSN is flat; Premature (`preempt_any`) RSN 195/223/206/215/232 vs CSN 266/196/206/139/220; ≥2 `Step` markers RSN 73/36/25/31/31 vs CSN 26/55/23/36/24. **No significance testing was run** (no McNemar/bootstrap vs α=0) — every number above is a raw point estimate from N=300, reported descriptively only; do not read the inverted-U or any single-cell swing (e.g. CSN `####`-at-start jumping to 58 at +4) as an established effect without a paired test. **Do not conflate this GSM8K pilot with the MMLU-E RR/CC line above** — different task, different launcher, different output tree, and this line's mask usage (raw NMD-format mask × integer α via the existing generic `--mask_type` mechanism) needed zero new mask-loading code, unlike RR/CC which needed a new script because `get_answer_regenerate_logits.py` cannot load an arbitrarily-named mask file. |
| Chat–Bare transition-subspace analysis (`RoleHidden/`, no protocol id) | **COMPLETE 2026-09-15, both models.** Tests whether Role direction (Expert−Non-expert, raw, never auto-flipped) lies within the low-dimensional PCA subspace spanned by SAMPLE-LEVEL Chat−Bare differences (900 = 300/task × 3 tasks [gsm8k/math/gsm_hard], equal-weighted), not just the mean Chat−Bare shift already shown near-zero dense cosine in the `4.full_role_chat_direction` line. Called the **"Chat–Bare transition subspace"** throughout — explicitly NEVER "the Chat manifold" (a linear PCA subspace of a finite sample, not a claim about the full nonlinear representational manifold). Script `RoleHidden/analyze_chat_role_transition_subspace.py` (~1200 lines; lives in the offline `RoleHidden/` workspace, NOT in this git repo). Reads existing H5/manifest only — **no hidden states re-extracted, nothing existing modified**. Three PCA subspace definitions: A (uncentered, diagnostic only — dominated by the mean shift), B (globally-centered), **C (within-task-centered, PRIMARY** — prevents cross-task mean differences from dominating). Cross-fitting: salted-SHA256 fold assignment (never Python `hash()`, which is process-salted), **exact 150/150 per task** by ranking digest values (not `hash%2`, which only balances in expectation), bidirectional evaluation (fit fold A → eval fold B and vice versa). Three null references, ≥1000 draws each: isotropic random direction and random k-dim subspace are the **same closed-form `R²(k) ~ Beta(k/2,(d-k)/2)` distribution** (sampled directly, no QR simulation), magnitude-preserving coordinate-permutation null stays a vectorized Monte Carlo (no closed form). Full-spectrum SVD (`np.linalg.svd(X, full_matrices=False)`) used for EVR-target k lookup; only a bounded `components_save` slice (≤50 rows) is written to the `.npz`, to avoid a real bug where truncating to `max(K_GRID)=50` BEFORE searching for the 80/90/95% EVR target silently mis-reports "k=50 reaches 80%" when the true k is far higher. Held-out reconstruction energy for definition C centers each task's held-out rows by **that task's own training-fold mean** (not by zero) before projecting, then averages the three tasks with equal weight. BLAS threads (`OMP/OPENBLAS/MKL/VECLIB/NUMEXPR_NUM_THREADS`) are **hard-set to `"1"`, not `os.environ.setdefault(...)`** — `setdefault` silently keeps a shared server's pre-exported high thread count instead of pinning it, which was the actual root cause of an initial ~70-minute-with-no-output hang on turing (compounded by, but not solely caused by, the null being QR-simulated rather than closed-form). All CSV/JSON/`.npz`/markdown-report outputs are atomic (tempfile + `os.replace`); the whole-run overwrite guard is checked before any computation and lists every output file including `pca_components_band.npz` and `band_level_null_summary.csv`. **TRUE band-level null distribution, not "average of each layer's own p97.5" (a real bug found and fixed 2026-09-15):** for each null type and k, the raw `n_draws`-length draw vector from every band `(layer, fold)` cell is averaged draw-index-by-draw-index into ONE band-level null-draw vector, and mean/p2.5/p97.5 are computed on THAT vector — a numerically distinct, stricter quantity from averaging each layer's own percentile (verified on a constructed two-population case: the two methods differ by ~0.01 in p97.5, confirming the fix is not a no-op). Written to `band_level_null_summary.csv` in every output dir, added to the overwrite guard, and is what the report's interpretation verdict reads — the old per-layer-averaged table is kept in the report but explicitly relabeled "DIAGNOSTIC ONLY". **RESULT (RSN band, k=10, definition C):** llama3 `[11,20)` centered R²=0.0788 vs TRUE band-level null p97.5=0.0030 (**32× enrichment**, held-out reconstruction energy 0.4381); qwen2.5 `[16,22)` centered R²=0.1261 vs null p97.5=0.0036 (**45× enrichment**, 0.4768). Both centered projections exceed BOTH TRUE band-level nulls' 97.5th percentile at every k∈{1,2,5,10,20,50}. **Frozen interpretation wording (do not soften or strengthen): "consistent with Role having a non-random geometric relationship to the Chat-Bare transition subspace" — explicitly NOT "shared causal mechanism", NOT "biological homology".** Ran on **turing** (not excluded after all — an earlier session-local instruction to avoid it was later explicitly retracted by the user); `nice -n 15`, single-threaded BLAS as above, completed in seconds per model once threads were actually pinned (vs. the earlier ~70 min hang with no output). Outputs synced to `AdaResult/5.chat_role_transition_subspace/{llama3,qwen2.5}/` (CSVs, `provenance.json`, `pca_components_band.npz`, `transition_subspace_report.md`). **Plots** (`RoleHidden/plot_chat_role_transition_subspace.py`, reads only the written CSVs, re-derives nothing): `plots/fig_main_role_projection.png` (MAIN — 1×2 panel per model, R²(k) vs k, centered solid / uncentered dashed, grey band = TRUE band-level null 2.5–97.5 pctile, k=10 enrichment annotated; paper-ready), `plots/fig_supp_layerwise_heatmap.png` (SUPP 1 — layer×k heatmap of centered R², RSN band boxed in red; **the elevated final-layer cell is captioned "should be interpreted cautiously because late-layer representations may exhibit distinct anisotropy and output-facing geometry; it is not treated as a mechanistic peak" — explicitly NOT attributed to "norm-growth artifact", since `R²=‖V_k^T r‖²/‖r‖²` is norm-invariant and per-layer PCA on a uniformly rescaled layer would not change directions, so norm growth cannot by itself explain an R² change**), `plots/fig_supp_enrichment.png` (SUPP 2 — top row absolute R² + null band, bottom row enrichment on a log scale, same model side by side; kept paired so a large enrichment at small k is never read without its absolute effect size, since enrichment mechanically falls with k as random-projection energy grows ~k/d — 116×→16× llama3, 98×→15× qwen2.5). |
| Reasoning RSN morphology + MRSN vs RRSN exact-NMD comparison (`RoleHidden/`, no protocol id) | **RETIRED 2026-09-17 — RRSN's DEFINITION CHANGED, see the `ARRSN redefined as RRSN` line below.** This entry's `RRSN` (3-task GSM8K+MATH+GSM-Hard unweighted-mean `role_diff`) is no longer the current RRSN and its `AdaResult/6.reasoning_rsn_morphology/` + `AdaResult/7.mrsn_rrsn_exact_nmd/` outputs were deleted by the user. Kept below as a historical record only — do NOT cite its band/overlap/cosine/containment numbers as current RRSN facts. **COMPLETE 2026-09-16, both models. TWO independent read-only scripts, no steering mask built, no steering run.** (1) **`RoleHidden/analyze_reasoning_rsn_morphology.py`** locates the candidate middle-layer band for the Reasoning RSN (`RRSN = Expert−Non-expert, raw, never auto-flipped`, unweighted mean over GSM8K/MATH/GSM-Hard task-level `role_diff`, no per-task L2 normalization) via layer-wise Expert/Non-expert Pearson correlation, independently of `~/CLAUDE.md`'s `RRSN`/MMLU-E band. Same layer/slot convention as everywhere else in this line (slot 0 = embedding, never a transformer layer; Llama transformer layers 1–32, Qwen 1–28; deliberately avoids `hs_analysis_aaai.py`'s `layer+1` offset trap, matches `pearson_aclarr.py`'s slot-0-removal convention). **Candidate-band detection uses `scipy.signal.find_peaks`+`peak_widths` on the negated Pearson series (rel_height=0.5), NOT a global percentile threshold** — a first percentile-threshold-based detector was found (via manual inspection, not user feedback) to conflate a genuine interior trough with a monotone tail-drift toward the last layer (this project's documented "end-layer amplitude artifact" trap), and in one case a contaminated global p25 excluded a real trough sitting a hair above its own threshold (Qwen layer-9: 0.9966 vs contaminated p25 0.99655). **Result: llama3 consensus candidate band (≥2/3 tasks) = `[7,13]`** (math alone diverges to `[14,16]`; intersection of all 3 is EMPTY); **qwen2.5 intersection=consensus = `[8,12]`** (all 3 tasks tightly agree). **Both candidate bands are EARLIER than and only partially/not-overlapping with the existing MMLU-E RSN band** (Llama `[11,20)` overlaps only at `[11,12,13]`; Qwen `[16,22)` is fully DISJOINT). **Explicit finding: `reasoning_mean` is closer to a (gsm8k, gsm_hard)-consensus direction than a balanced 3-task blend** — cosine(reasoning_mean, per-task direction) shows math as a clear outlier (llama3 0.53, qwen2.5 0.63) vs gsm8k/gsm_hard (0.94–0.95 both models), a 2-vs-1 split not single-task domination. Labeled **EXPLORATORY throughout** — candidate band selected from representation structure only, never from downstream accuracy or existing steering results, and explicitly NOT frozen as a steering band. Outputs: `AdaResult/6.reasoning_rsn_morphology/{llama3,qwen2.5}/*.csv`, `provenance.json`, `reasoning_rsn_morphology_report.md`, 9 PNGs under `plots/`. **Plotting variants (2026-09-16, read the already-written CSVs only, nothing re-derived):** `RoleHidden/plot_reasoning_rsn_morphology_single_task.py` reproduces `pearson_aclarr.py`'s exact ACL-ARR rcParams/single-muted-blue-curve/no-MMLU-E-overlay style, one task per figure (`--task {gsm8k,math,gsm_hard,reasoning_mean}`), output `AdaResult/6.reasoning_rsn_morphology/plots_single_task/<task>/pearson_<model>.png` — built to visually confirm the candidate band without the multi-series overlay competing for attention. (2) **`RoleHidden/analyze_mrsn_rrsn_exact_nmd.py`** is a SEPARATE static structural comparison between the MMLU-E RSN (`MRSN = diff_mean−none_diff_mean`) and this RRSN, both reduced to their **exact-NMD sparse support** (`top_k=int(H*0.5/100)`: Llama 20, Qwen **17, NOT 18 and NOT `round(H*0.005)`** — an explicit correction from an earlier analysis's rounding convention) over each model's **existing MMLU-E-matched band only** (Llama `[11,20)`, Qwen `[16,22)`; RRSN's own native band from script (1) above is deliberately NOT used here and NOT re-selected). **RRSN aggregation identity re-verified independently** (recomputed `mean(role_diff_gsm8k,math,gsm_hard)` from the three per-task `AdaResult/2.constructed_directions/*.npy` files and compared to the saved `reasoning_mean` array: max_abs_err = 0.0 exactly, both models) plus a live cross-check of `constructed_directions_manifest.json`'s sign/aggregation-rule TEXT against the expected strings. **Both MRSN's and RRSN's exact-NMD masks are cross-checked against a live import of `detection/nmd.py::get_nmd_mask`** (never re-derived from memory), index-and-signed-value-exact, both PASS on both models. Three metric families, computed per-layer AND pooled over the band by summing numerator/denominator across `(layer,neuron)` coordinates before dividing (never averaging per-layer ratios): (a) **support overlap** — pooled Jaccard/overlap_coefficient/enrichment-vs-random(`top_k²/H` scaled by n_layers)/equal-size hypergeometric upper-tail p; (b) **sparse weighted cosine** computed over the FULL union coordinate space (each mask's own full norm as denominator, dot product only nonzero on shared support) plus auxiliary shared-coordinate-only sign-agreement/cosine (written `NA`, never `0`, when no shared coordinate exists); (c) **bidirectional dense-energy containment** `C(R←S_M)=Σ RRSN_dense[S_M]²/Σ RRSN_dense[:]²` and `C(M←S_R)` symmetric, plus self-capture ceilings `C(M←S_M)`/`C(R←S_R)` and relative efficiencies `C(R←S_M)/C(R←S_R)` and `C(M←S_R)/C(M←S_M)`. **RESULT: pooled overlap far above random but energy containment well below each direction's own self-capture ceiling** — llama3 `[11,20)`: overlap 33/327 (Jaccard 0.101, **37.5× enrichment**, hypergeom p=1.8e−42), pooled sparse weighted cosine 0.256, shared-coordinate sign agreement 0.788; `C(R←S_M)=0.045` vs `C(R←S_R)` self-cap `0.135` (relative efficiency 0.333), `C(M←S_R)=0.049` vs `C(M←S_M)` self-cap `0.094` (relative efficiency 0.517). qwen2.5 `[16,22)`: overlap 14/190 (Jaccard 0.074, **28.9× enrichment**, hypergeom p=4.9e−17), pooled weighted cosine 0.199, shared sign agreement 0.929; `C(R←S_M)=0.060` vs self-cap `0.173` (rel. eff. 0.345), `C(M←S_R)=0.029` vs self-cap `0.087` (rel. eff. 0.334). **Frozen interpretation, do not soften or strengthen: "structural/sparse-coordinate relationship" only — explicitly NOT functional equivalence, NOT a shared causal mechanism, NOT evidence RRSN would transfer to MMLU-E under steering** (that requires a separate norm-matched cross-steering experiment, not run here). Outputs: `AdaResult/7.mrsn_rrsn_exact_nmd/{mrsn_rrsn_exact_nmd_layerwise.csv, mrsn_rrsn_exact_nmd_band_summary.csv, mrsn_rrsn_shared_coordinates.csv, provenance.json, mrsn_rrsn_exact_nmd_report.md, mrsn_rrsn_layerwise_summary.png}` (single simple layerwise summary figure only, deliberately NOT extended into a Manifold/subspace analysis). All outputs atomic-write, whole-run overwrite guard checked before any computation (verified: re-running after a completed run refuses cleanly with zero partial writes). |
| RRSN → GSM8K own-direction positive control (`RoleHidden/` + `gsm8k_rrsn_positive_control/`, no protocol id) | **RETIRED 2026-09-17 — built against the OLD RRSN (3-task unweighted-mean `role_diff`), which no longer exists as RRSN's definition (see `ARRSN redefined as RRSN` below).** `AdaResult/8.rrsn_gsm8k_positive_control/` masks were built from the old RRSN and were never actually steered on GPU (see "NOT YET RUN" below) — do not deploy them or resume this line without first rebuilding the mask from the new RRSN. Kept as a historical record of the mask-build methodology (norm-matching procedure, `.npy` suffix bug fix) only. **MASK-BUILD COMPLETE + LOCALLY VERIFIED 2026-09-16; GENERATION CODE COMPLETE, NOT YET RUN (no GPU access this session).** Question: does RRSN (Reasoning RSN — unweighted mean of the 3 per-task GSM8K/MATH/GSM-Hard Expert−Non-expert `role_diff`, raw sign), reduced to its exact-NMD sparse support over each model's EXISTING MMLU-E-matched band (Llama `[11,20)`, Qwen `[16,22)` — RRSN's own native/candidate band from `analyze_reasoning_rsn_morphology.py` deliberately NOT used) and **per-layer L2-norm-matched to that model's own MRSN exact-NMD support**, have causal activity on GSM8K when steered the same way MRSN is? Framed as an **IN-DOMAIN positive control for RRSN, NOT a cross-task generalization test** — GSM8K is one of the 3 tasks RRSN was constructed from; a positive result shows matched-band RRSN CAN move GSM8K behaviour, nothing about transfer; a null must first be checked against band/dose/injection validity before being read as "RRSN and MRSN differ functionally". No MMLU-E steering run, no Manifold extension, per explicit scope. **Step 1 (mask build, offline, no GPU, DONE):** `RoleHidden/build_rrsn_gsm8k_positive_control_mask.py` — builds MRSN and RRSN exact-NMD masks (top_k=20 Llama/17 Qwen, cross-checked against a live import of `detection/nmd.py::get_nmd_mask`, both directions PASS both models), then rescales each in-band RRSN row so `‖RRSN_scaled_row_l‖₂ == ‖MRSN_row_l‖₂` (verified rel_err < 1e-5 on every layer, both models — matches per-layer MAGNITUDE only, never direction). RRSN aggregation identity re-verified independently (max_abs_err=0.0, both models). Output `(L-1,H)` masks saved in the shape `VicundaModel.regenerate`'s `diff_matrices` expects (one row per decoder layer, matching `get_nmd_mask`'s embedding-row-dropped convention) — llama3 `(32,4096)` 9 nonzero rows, qwen2.5 `(28,3584)` 6 nonzero rows, both verified. **Caught and fixed a real bug while building this**: an initial `atomic_write_npy` used `mkstemp(suffix=".npy.tmp")`, but `np.save` unconditionally appends `.npy` to any path not already ending in exactly `.npy`, so it silently wrote the real array to `tmp_path.npy` while `os.replace` moved the untouched EMPTY `tmp_path` into place — every mask file was 0 bytes until this was found (`np.load` raising `EOFError`) and fixed (suffix changed to plain `.npy`, plus an explicit post-write nonempty-file assertion). Output: `AdaResult/8.rrsn_gsm8k_positive_control/masks/{rrsn_scaled,mrsn_reference}_{model}_{size}.npy` + `rrsn_gsm8k_positive_control_mask_provenance.json` (this LOCAL archive path is never rewritten, renamed, or deleted). **Step 2 (generation, server GPU, code complete, NOT RUN):** new independent `gsm8k_rrsn_positive_control/get_answer_gsm8k_rrsn_positive_control.py` + `run_gsm8k_rrsn_positive_control.sh` (`--baseline`/`--sweep`/`--full`) — does NOT modify `get_answer_regenerate_gsm8k.py`, `run_gsm8k.sh`, or `run_gsm8k_qwen25.sh`; a new script was needed because the existing driver can only load a mask by its fixed `{mask_type}_{percentage}_{start}_{end}_{size}.npy` filename convention and cannot load an arbitrary pre-built per-layer-scaled matrix. **Mask deployment path (fixed 2026-09-16, no data/protocol change): the generation script loads the mask from the SERVER's standard mask tree, `${base_dir}/mask/{model}_non_logits/rrsn_normmatched_0.5_<start>_<end>_<size>.npy`** — the same directory every other steering launcher already uses for its MRSN mask (`nmd_0.5_<start>_<end>_<size>.npy`), rather than a bespoke RoleHidden-relative path. Deploying is a plain file copy of the local archive's `rrsn_scaled_{model}_{size}.npy` array content under this new server filename — it never overwrites the existing `nmd_0.5_*.npy` MRSN masks in the same directory. **Verified deployed 2026-09-16: uploaded to `turing`, server-side array-content sha256 matches the local archive exactly for both models** (`3f8da44c...` llama3, `b28d5f19...` qwen2.5); the pre-existing `nmd_0.5_*.npy` MRSN masks in the same two directories were confirmed untouched (unchanged mtimes). **The runtime dependency on `--rolehidden_dir` / the server-side provenance JSON was removed entirely** — the driver verifies the deployed mask exclusively from its own array content (shape, all-finite, band-aligned nonzero rows, and EXACTLY the model's frozen top_k (20 Llama/17 Qwen) nonzero entries in every in-band row, zero elsewhere — all checked directly, no external record needed) and computes its actual sha256 at load time, writing it into `run_config.json` for offline cross-checking against the local archive's own recorded array sha256 (never against a server-side copy of the provenance file, which is not uploaded and stays a local-only audit artifact). **The standalone `--check` preflight mode was REMOVED (2026-09-16)** — mask validation runs automatically at the start of every formal generation invocation (`--baseline`/`--sweep`/`--full`), before the model loads; there is no separate preflight/check command, and the launcher's usage line lists only `{--baseline|--sweep|--full}`. Reproduces each model's frozen No-CoT GSM8K pipeline EXACTLY otherwise: same 300-question benchmark+order, neutral/Bare/No-CoT plain-wording prompt (`select_templates_gsm8k(suite="default", cot=False, wording="plain")`), post-`<|eot_id|>` terminators, prefill-only last-token injection (`vc.regenerate` default `prefill_tail_len=1`), greedy/`max_new_tokens=768`/`batch_size=24`, and each model's SAME frozen alpha grid (Llama `−8..+8` step 2, Qwen `−8..+12` incl. `+10/+12`) — the ONLY difference from the existing pipeline is which mask is loaded. Fail-closed per cell, performed automatically before any generation: mask shape/finite/band/exact-top_k verified directly from the array (refuses on mismatch — guards against a stale/hand-edited mask); requested α must be in the frozen grid; α=0 must register exactly 0 `steering_fire_count()` (a real all-zero `diff_matrices` still registers hooks, firing 0 only because every row is zero — same convention as every other GSM8K launcher in this repo); α≠0 must register exactly `n_layers_in_band × 300 × 1` fires; benchmark file existence, sample count, and questions sha256 recorded; whole-cell overwrite guard (refuses unless `--allow_overwrite`); every model's ENTIRE alpha grid including a freshly-re-run α=0 must complete on ONE card (bf16 greedy not byte-reproducible across GPUs — this curve's own baseline is never copied from the existing frozen GSM8K tree). Output `components/{model}/gsm8k_rrsn_positive_control/mdf_<alpha>/{gsm8k_rrsn_pc_{size}_answers_<top_k>_<start>_<end>.json, run_config.json}` (run_config.json carries mask path, mask sha256, steering_fires, prompt template, full generation args per cell — inline accuracy explicitly labelled NOT the formal metric). **Step 3 (offline accuracy scoring, code complete, NOT RUN):** `RoleAnswer/rrsn_positive_control/eval_gsm8k_rrsn_positive_control.py` — imports `all_hash`/`norm_gsm8k`/`fallback_gsm8k`/`first_last_stats` from `analyze_first_last_acc.py` (never reimplemented), `first_acc` MAIN / `last_acc` sensitivity-only, missing answers stay in the n=300 denominator. Each non-zero α paired against THIS curve's own α=0 (same card), exact two-sided McNemar + Holm-Bonferroni across the full non-zero-α family (`mcnemar_exact`/`holm` copied VERBATIM from `p3/run_p3_eval.py`'s existing frozen pattern, since that module is import-gated behind its own frozen-prediction guard) — ALL doses reported including null/degradation, **workpoint never redefined post-hoc from the observed best cell**. Fail-closed on missing cell / n≠300 / sample-order misalignment vs α=0. **Step 4 (behavioral summary, code complete, NOT RUN):** `RoleAnswer/rrsn_positive_control/unified_behavior_gsm8k_rrsn_positive_control.py` — imports `compute_row` BY IMPORT from `unified_behavior_gsm8k_math.py` (zero redefinition): candidate timing, valid submission, FIRST/LAST revision, loop/truncation (via candidate coverage), generation-length metrics and their respective denominators are IDENTICAL to that script's GSM8K row computation. Descriptive/post-hoc only — no significance testing, no new mechanism claims, text position never read as internal commitment or causal mediation. Full runbook + exact commands for steps 2–4: `RoleHidden/AdaResult/8.rrsn_gsm8k_positive_control/RUNBOOK.md`. **No GPU has been launched for this line yet.** |
| GSM8K abstention-enabled Role HS extraction (`gsm8k_abstention_role`, no protocol id) | **HS EXTRACTION + SERVER-SIDE MEAN/DIFF COMPLETE 2026-09-17, GPU RUN DONE (llama3 + qwen2.5); NO downstream use yet (no layer band, no NMD mask, no RRSN/MRSN analysis — deliberately deferred).** Minimal-scope GSM8K-only (fixed 300-question benchmark, original order) Role HS extraction for Llama-3.1-8B-Instruct and Qwen2.5-7B-Instruct, adding "honest" + an identical abstention exit sentence (`If you are not sure, you may answer exactly "I am not sure" instead.`) to BOTH roles — deliberately NOT reusing the old no-honest GSM8K Role prompt, to better match the MMLU-E role framing. New standalone code: `extract_gsm8k_abstention_role_hs.py` + `run_gsm8k_abstention_role_{llama3,qwen25}.sh` (prefill-only, no generation/steering/hooks; verifies the derived prompt is a minimal diff from `build_gsm8k_default_suite(cot=False, wording="plain")["neg"]`; hard-checks the ordered-sample-identity pairing digest against the KNOWN value from the old GSM8K Role extraction, `64af9b38dac72f35ee1efa97bea8026e0c4d97b35730e76361eea9581a23cf35`, dies on mismatch; `--wording` locked to `plain` only) — does NOT touch `extract_role_hidden_states.py` or its launchers. Server output `components/hidden_states/{model}/gsm8k_abstention_role/{expert,non_expert}_{size}.h5` + ONE experiment-level `manifest.json`. **`mean/mean_gsm8k_abstention_role.py`** computes the server-side means/diff: means are float32-accumulated then cast to float16 for saving (`mean_role_reasoning.py`'s convention), and the DIRECTION is computed by upcasting those SAVED float16 means back to float32 and subtracting (round-then-subtract, matching the existing formal Role-direction construction and the old GSM8K Role direction file's dtype) — saved as float32, NOT float16 — rather than subtracting the pre-cast float32 means (subtract-then-round), which was verified to differ at ~3.8% of positions on a synthetic fixture. `diff = expert_mean − non_expert_mean`, raw sign, no L2 normalization. Manifest records `mean_dtype`/`diff_dtype` separately (not a unified `output_dtype`), updated additively (every pre-existing key preserved). **Server outputs migrated 2026-09-17 into the flat `hidden_states_mean/{model}/` convention** (parallel to the existing MMLU-E `mmlue_expert_mean_*`/`mmlue_non_expert_mean_*` — renamed from the legacy `diff_mean_*`/`none_diff_mean_*` at the same time): `hidden_states_mean/{llama3,qwen2.5}/gsm8k_abstention_{expert,non_expert}_mean_{8B,7B}.npy` + `gsm8k_abstention_role_diff_{8B,7B}.npy`. The original H5 + experiment manifest under `components/hidden_states/{model}/gsm8k_abstention_role/` remain the authoritative provenance record (prompt text, pairing digest, sha256) and were NOT touched by the migration. **Qwen2.5 has NOT run the MMLU-E confidence-neuron line** (`mmlue-confidence-hs-v0` is Llama3.1-8B-Instruct only) — `hidden_states_mean/qwen2.5/` has no `mmlue_confidence_*` files, only `mmlue_*` (role) and `gsm8k_abstention_*`. No layer band, no NMD mask construction, and no RRSN/MRSN similarity analysis using this new `gsm8k_abstention_role_diff` — explicitly deferred pending a new research direction, per this line's original scope. |
| ARRSN redefined as RRSN (`RoleHidden/`, no protocol id) | **REDEFINITION, 2026-09-17.** The user deleted all outputs of the OLD `RRSN` line (3-task unweighted-mean `role_diff` over GSM8K/MATH/GSM-Hard `AdaResult/2.constructed_directions/*.npy`, `AdaResult/6.reasoning_rsn_morphology/`, `AdaResult/7.mrsn_rrsn_exact_nmd/`, `AdaResult/8.rrsn_gsm8k_positive_control/` — see the two RETIRED lines above) and **RRSN now means the full GSM8K abstention-enabled ARRSN** (the construction-split direction from the `GSM8K abstention-enabled Role HS extraction` line immediately above: `ARRSN_final = gsm8k_abstention_expert_mean − gsm8k_abstention_non_expert_mean`, n≈1019 construction-split questions, round-then-subtract, raw sign). **Other reasoning tasks (MATH, GSM-Hard) are explicitly EXCLUDED from RRSN for now** — RRSN is GSM8K-only until further notice; do not re-blend MATH/GSM-Hard `role_diff` into it without a new explicit instruction. `RoleHidden/analyze_arrsn_mrsn_dense_comparison.py` and `RoleHidden/analyze_arrsn_mrsn_exact_nmd.py` (the scripts written for the "ARRSN vs MRSN" comparison, see the GSM8K abstention-enabled Role HS extraction line's construction-split follow-up) ARE this redefinition's current RRSN-vs-MRSN comparison pipeline — their `AdaResult/9.arrsn_mrsn_dense_comparison/` and `AdaResult/10.arrsn_mrsn_exact_nmd/` outputs (re-run 2026-09-17 against the construction-split means, replacing the earlier 300-question-pilot-based versions) are the CURRENT RRSN vs MRSN result, superseding the retired `7.mrsn_rrsn_exact_nmd/` numbers. Inputs verified before that re-run: both models' `gsm8k_abstention_{expert,non_expert}_mean_*`/`gsm8k_abstention_role_diff_*` in `{llama3,qwen2.5}_logits/matrix/` pass shape (`(1,1,33,4096)` llama3 / `(1,1,29,3584)` qwen2.5), dtype (mean fp16, diff fp32), all-finite, and exact identity (`expert.astype(f32) - non_expert.astype(f32) == diff`, max_abs_err=0.0 both models) checks. **RESULT (all transformer layers, no band pre-selection, exact-NMD top_k=20 llama3/17 qwen2.5):** llama3 overlap_coefficient=0.1844, jaccard=0.1015, enrichment=37.76×, hypergeom p=6.98e−148, pooled sparse weighted cosine=0.1099, `C(A←S_M)=0.0337` vs self-cap `C(A←S_A)=0.0746` (rel. eff. 0.4521), `C(M←S_A)=0.0367` vs self-cap `C(M←S_M)=0.0753` (rel. eff. 0.4877); qwen2.5 overlap_coefficient=0.1786, jaccard=0.0980, enrichment=37.65×, hypergeom p=1.08e−106, pooled cosine=0.2235, `C(A←S_M)=0.0889` vs self-cap 0.1677 (rel. eff. 0.5300), `C(M←S_A)=0.0446` vs self-cap 0.1051 (rel. eff. 0.4244). Same frozen interpretation boundary as the retired line: structural/sparse-coordinate relationship only — NOT functional equivalence, NOT a causal mechanism, NOT evidence of steering transfer. **No downstream RRSN→GSM8K positive-control mask has been rebuilt against this new RRSN** — the retired `8.rrsn_gsm8k_positive_control/` masks are stale and must not be reused; rebuilding that mask (or resuming any RRSN steering line) requires re-running `build_rrsn_gsm8k_positive_control_mask.py`-equivalent logic against the new `gsm8k_abstention_role_diff_*` files first. |
| GSM8K construction-split Chat direction (`gsm8k_construction`, no protocol id) | **CODE COMPLETE + LOCALLY VERIFIED 2026-09-17; NOT YET RUN ON GPU.** Standalone GSM8K-only sibling of `extract_interface_hidden_states.py`/`mean/mean_diff_chat.py` (the existing 300-question, 3-task Chat/Bare interface pipeline whose `Chat–Bare transition-subspace analysis` line above documents the downstream use), built so the Chat direction (`Chat_direction = chat_mean − bare_mean`) can be compared against the redefined RRSN on a **sample-composition-matched** basis — both built from the SAME `components/benchmark/gsm8k_construction_split.json` (n≈1019, same question set/order as `ARRSN_final`), rather than the existing frozen 300-question chat/bare files (whose sample set is the DISJOINT reserved set, by construction). New code: `extract_interface_hidden_states_construction.py` (per-cell `--model {llama3,qwen2.5} --condition {chat,bare}`, GSM8K-only — no `--task` flag, no MATH/GSM-Hard) + `run_interface_hs_construction_{llama3,qwen25}.sh` (2 cells each: chat+bare) + `mean/mean_diff_chat_construction.py` (single-task sibling of `mean/mean_diff_chat.py`, `n` read from data not hardcoded to 300, cross-checked for chat/bare shape+pairing-digest equality). **Every prompt/model/layer-storage convention is copied byte-for-byte from the parent script** (same `MODELS` dict incl. `expected_tail_bare`/`expected_tail_chat` token id/text pairs, same `build_gsm8k_default_suite(cot=False, wording="plain")["neutral"]` body builder, same `apply_chat_template`+`strip_leading_bos` chat wrapping, same prefill-only all-layers-stored convention, same fail-closed final-token/no-double-BOS/finite checks) — the ONLY differences are: GSM8K-only scope, the construction-split question file (`n` cross-checked against `gsm8k_construction_split_manifest.json`'s own `construction_split_n` AND `construction_split_ordered_sample_identity_sha256`, dies on mismatch), and a separate non-colliding output tree `components/hidden_states/{model}/gsm8k_construction/{condition}_{size}.h5` (distinct from both the parent script's `.../gsm8k/{condition}_{size}.h5` and the ARRSN construction H5 pair under `.../gsm8k_abstention_role_construction/`). Output mean/diff: `components/hidden_states_mean/{model}/gsm8k_construction/{chat_mean,bare_mean,diff_mean}_{size}.npy` (**per-model top-level directory, matching the existing `hidden_states_mean/{model}/` layout used everywhere else in this repo — NOT `{model}_chat_construction`**, since the `gsm8k_construction` task subdirectory alone already disambiguates from every other task) (fp16, `(1,1,n_layers,hidden_size)`, `diff_mean = chat_mean − bare_mean`, round-then-subtract-free — direct elementwise mean over the sample axis then subtract, matching `mean_diff_chat.py`'s own convention exactly, NOT the abstention-role mean scripts' float16-then-upcast round-then-subtract convention, since this pipeline never separately saves means before computing the diff). **`mean_diff_chat_construction.py`'s per-cell validation is LOCKED, not just shape/digest-based (tightened 2026-09-17 after review):** `--size` must match the model's own locked size (llama3=8B, qwen2.5=7B); `n_samples` must equal exactly 1019 (the locked construction-split count, not merely "positive"); each H5's own `model`/`task`/`condition` attrs must agree with the file actually being read (catches a misplaced/mislabeled H5, e.g. the 300-question tree copied into this path by mistake); `generation_performed` must be False and `prefill_only` True; `chat_template_applied` must match its own condition (True for chat, False for bare — catches a chat/bare mislabeling that would silently compute the diff backwards or from two files of the same condition); `questions_sha256` and `construction_split_manifest_digest` must agree between chat and bare, in addition to the existing `ordered_sample_identity_sha256` pairing check. The extractor's own manifest cross-check was also tightened: a MISSING `construction_split_ordered_sample_identity_sha256` key in the split manifest (not just a mismatched one) now correctly dies rather than silently passing (`if not manifest_digest or manifest_digest != pairing_digest`, was `if manifest_digest and ...`). **Locally verified (no GPU): `py_compile`/`bash -n` clean; `--print_pairing_digest_only` run against the real local construction-split fixture reproduces `n=1019` and digest `cb7f8bf6639e8b728ed6fa0b1af182c0c8f98df62a316d0cc6423d34bbf3c8b3` — matching the split builder's own recorded digest exactly, and identical across both models' launcher invocations (cross-model consistency of the construction question set); both `run_interface_hs_construction_{llama3,qwen25}.sh` driven with real arguments (not just `bash -n`) reach model-loading before terminating locally (no GPU available), confirming path resolution/digest cross-check/output-path construction all correct.** Mutation-tested (2026-09-17, each check individually, against a valid n=1019 fixture with one attr corrupted at a time): missing split-manifest digest key, `n_samples_done` mismatch, wrong `model`/`task`/`condition` attrs, `generation_performed=True`, `prefill_only=False`, `chat_template_applied` mismatched to its own condition, `questions_sha256` mismatch, `construction_split_manifest_digest` mismatch, `n_samples≠1019`, and wrong `--size` for the locked model — all 11 correctly refuse with a specific, distinct error message and write nothing; `mean_diff_chat_construction.py`'s overwrite guard also correctly refuses a re-run listing all 4 existing output paths. **Explicit interpretation-boundary caveat (from review, not a code issue): `Chat_direction = chat_mean − bare_mean` still confounds the native-chat-template wrapping itself with the accompanying change in final prefill anchor/token (bare ends on `Answer: `/token 220; chat ends on the assistant-header token, 271 for llama3 / 198 for qwen2.5 — see `extract_interface_hidden_states.py`'s own documented confound). This direction must NOT be described as a pure "Chat semantic direction" — it mixes template-wrapping semantics with injection-site/anchor-token geometry, same caveat the `GSM8K/MATH chat matched-anchor control` line below documents for a different pipeline.** **No GPU has been launched for this line yet — the assistant does not run GPU jobs.** |
| GSM8K abstention-role NO-STEERING behavioral audit (`gsm8k_abstention_role_audit`, no protocol id) | **CODE COMPLETE + LOCALLY VERIFIED 2026-09-17; NOT YET RUN ON GPU.** Minimal, standalone, GSM8K-only behavioral audit: free-form generation (NOT hidden-state extraction) for the ARRSN HS extraction's expert-vs-non_expert abstention-enabled prompt, on the SAME fixed 300-question benchmark, with NO steering and NO mask loaded — asks whether non-expert framing measurably raises abstention/lowers coverage in actual model OUTPUT, as a behavioral complement to the HS-level ARRSN direction. New code: `get_answer_gsm8k_abstention_role_audit.py` (generation + parsing) + `analyze_gsm8k_abstention_role_audit.py` (offline statistics) + `run_gsm8k_abstention_role_audit_{llama3,qwen25}.sh`. **Prompt is IMPORTED, not retyped**: `build_abstention_templates`/`render_prompt`/`ABSTENTION_ROLE_TO_CHARACTER`/`ABSTENTION_SENTENCE`/`CONDITIONS`/`N_EXPECT`/`EXPECTED_GSM8K_PAIRING_DIGEST` are imported directly from `extract_gsm8k_abstention_role_hs.py` — the deliberate OPPOSITE choice from the construction-split HS sibling's "duplicate, don't import" pattern, since here the audit is meant to behaviorally test THAT SAME prompt, so prompt drift between the two must be structurally impossible rather than merely checked. Pairing digest checked against the same `EXPECTED_GSM8K_PAIRING_DIGEST` (`64af9b38...`) that script uses. **Generation uses `VicundaModel.generate()`, NOT `vc.regenerate()`** — `generate()` registers no forward hooks anywhere and never references a mask/diff matrix (verified by reading `llms.py` before writing this script), making "no steering" structural rather than "mask defaults to zero" (the latter is `regenerate`'s own α=0 convention documented elsewhere in this file as still registering real, if all-zero, hooks). Same generation config as the frozen `run_gsm8k.sh`/`run_gsm8k_qwen25.sh` No-CoT main line: `max_new_tokens=768`, `temperature=0.0` (greedy), `batch_size=24` (verified from both launchers, not assumed). **Four mutually-exclusive outcome categories per sample** (`correct`/`wrong`/`abstained`/`invalid`), computed from the FIRST `####` marker only (matching this repo's FIRST-main production convention, deliberately NOT `utils.extract_gsm8k_answer`'s loose multi-pattern fallback chain, which would convert genuine format failures into apparent wrong answers and destroy the `invalid` category); `abstained` requires the EXACT phrase `"I am not sure"` (case-sensitive substring, sliced from the imported `ABSTENTION_SENTENCE` constant) with NO parseable `####` marker anywhere — **precedence rule: a parseable marker always wins over the abstention phrase** (a sample containing both "I am not sure" and a `####` value is scored correct/wrong, never abstained, since it did commit to a number). LAST-marker correctness also recorded per row as a sensitivity-only field. **Offline analysis reports, per role:** `first_acc` (abstained/invalid count as incorrect, full n=300 denominator), `last_acc` (sensitivity only), `abstention_rate`, `coverage=1-abstention_rate`, `conditional_acc` (denominator = rows with outcome in {correct,wrong} only, reported explicitly since it differs by role), `invalid_rate`. **Paired expert-vs-non_expert comparison**: paired bootstrap 95% CI (B=10000, resampling question-index pairs so a question's two role-outcomes always travel together) on each of first_acc/abstention_rate/coverage/invalid_rate deltas, PLUS three SEPARATE exact McNemar tests (imported from `p3/run_p3_eval.py::mcnemar_exact`, not reimplemented) — one on first_acc, one on the `abstained` indicator specifically, one on the `invalid` indicator specifically. **The abstained-vs-invalid McNemar split is the direct answer to "higher abstention, not just more format errors"** — the two p-values are reported side by side rather than as one aggregate, so a non_expert effect that is really just more format failures cannot masquerade as a true abstention effect. **Fail-closed (both scripts)**: generation script — overwrite preflight before model load, n==300, pairing-digest match, model-shape lock, atomic write, every row stamps `steering_applied:false`/`mask_loaded:false`/`generation_method:"VicundaModel.generate"`; analysis script — refuses a steered/masked run_meta, requires exactly 300 rows per condition with a full 0..299 idx cover and no duplicates, structurally re-verifies `question`+`gold_answer` identity between the expert/non_expert row at every idx (not merely trusting the stored digest), rejects any row with an outcome outside the four expected categories, refuses to overwrite existing output. **Locally verified (no GPU): `py_compile` clean on all four files; 8 hand-written `parse_outcome` unit tests pass (all four categories, the precedence rule, multi-marker first-vs-last divergence, comma-formatted numbers); `--verify_only` run against the real 300-question benchmark reproduces the exact `EXPECTED_GSM8K_PAIRING_DIGEST` and prints byte-identical-except-role-wording prompts for both models; mutation-tested — corrupting one question's text and truncating to 299 samples each correctly refuse before any generation; the offline analyzer was run end-to-end against a synthetic 300-row fixture DELIBERATELY DESIGNED with non_expert abstaining far more than expert (81 vs 13 of 300) but an IDENTICAL invalid rate (13 vs 13) — the analyzer correctly reported `abstained` McNemar p=6.8e−21 (highly significant) alongside `invalid` McNemar p=1.0 (no difference), demonstrating the abstention-vs-format-failure split works as designed; separately mutation-tested the analyzer's overwrite guard, steered-run refusal, question-pairing-mismatch refusal, wrong-row-count refusal, and invalid-outcome-value refusal — all five correctly refuse with a specific message; both launchers driven with real arguments (not just `bash -n`) via `--verify_only` reproduce the identical prompt/digest output for both models, and the full (non-`--verify_only`) invocation was launched in the background and confirmed to reach the numpy/torch import check before being killed (no local GPU).** **No GPU has been launched for this line yet — the assistant does not run GPU jobs.** |
| GSM8K/MATH chat matched-anchor control (`{math,gsm8k}-chat-matched-anchor-v1`) | **CODE + TESTS COMPLETE, PREPARED BUT NOT RUN, 2026-09-12.** Question: `get_answer_math_chat_sweep.py`/`get_answer_gsm8k_chat_sweep.py`'s own docstrings record a MEASURED confound — native chat wrapping moves the last prefill token (the only position prefill-only tail=1 steering injects into) from the bare condition's `Answer: ` anchor (id 220, `' '`) to the assistant generation header (id 271, `'\n\n'`), because `apply_chat_template`'s Jinja `| trim` strips the anchor's trailing space. This control keeps the FULL chat template but manually re-attaches the `Answer: ` anchor directly after the assistant header, recovering token id 220/`' '` UNDER the chat wrapping — verified on the real cached `meta-llama/Llama-3.1-8B-Instruct` tokenizer, not assumed. **Frozen wording, and it is deliberately weaker than an earlier draft's**: matching the final prefill token's id and anchor does NOT mean matching its hidden state — surrounding context and absolute position still differ, and more than one token differs between matched-anchor and bare — so any recovered effect must be worded as "sensitive to the assistant-side anchor/injection geometry," never as "caused by" or "fully explained by" the injection-site shift alone. Full NINE-point α sweep `{-8,-6,-4,-2,0,+2,+4,+6,+8}` for BOTH tasks (the same dose family as the frozen bare/native-chat curves, not a re-search); MATH keeps its own budget (2048/bs=8), GSM8K keeps its own (768/bs=24); n=300, layers `[11,20)`, prefill-only tail=1, one model load per task running all nine α sequentially. **MATH and GSM8K are INDEPENDENT statistical families — own Holm m=8 each, corrected separately, NEVER combined into one m=16 family — and may run CONCURRENTLY on two GPUs** (the GSM8K launcher's earlier MATH-completion/`CONFIRMED` gate was deleted; verified by direct execution that it now proceeds straight to model loading with zero MATH-related logic). Bare/native-chat comparisons stay descriptive only, never significance-tested against matched-anchor; MATH's bare tree covers only 5 of the 9 doses and is labelled a `5/9 descriptive subset`. Shared delicate logic (anchor stripping, double-BOS guard, tail-token assertion, alpha-family validation) lives in ONE module, `chat_matched_anchor_lib.py`, imported by both generators — so the invariant cannot silently drift between tasks the way two independent copies could. Fail-closed, no-escape-hatch design: no `--allow_overwrite` flag; all nine output paths per task are pre-checked before any cell runs, refusing the WHOLE task if any exists. Verified before this line was written that no real matched-anchor output exists anywhere in either repo, so the protocol name `{math,gsm8k}-chat-matched-anchor-v1` was safely reused for the expanded nine-point family rather than version-bumped. Code: `chat_matched_anchor_lib.py`, `get_answer_{math,gsm8k}_chat_matched_anchor.py`, `run_{math,gsm8k}_chat_matched_anchor.sh`, `test_chat_matched_anchor.py` (Group A fake-tokenizer guard tests, Group B real-tokenizer three-condition token verification, Group C alpha-family mutation tests: missing/duplicate/extra/float dose rejected, shuffled order accepted); analyzer `RoleAnswer/analyze_chat_matched_anchor.py` + `RoleAnswer/test_analyze_chat_matched_anchor.py` reuse the frozen `RoleAnswer/analyze_first_last_acc.py` / `RoleAnswer/chat_sweep_common.py` extractors and statistics verbatim rather than reimplementing them (**all four of these live in the offline `RoleAnswer/` workspace, NOT in this git repo** — a server `git pull` fetches none of them); native-chat comparison root corrected to the real on-disk paths (`llama3/{math,gsm8k}/chat`, not `llama3/{math,gsm8k}_chat_v1`). **No GPU has been launched for this line yet** — code, static checks and synthetic-fixture tests only. |
...
are exploratory and outside it. Every registered new-vs-stored contrast is a CROSS-RUN
pairing because the stored cells have no recoverable device field; do not silently describe
them as same-GPU cells. Full statistics and CIs: `docs/wps_evaluation.json`.


<!-- Why: every doc reference, prereg and launcher comment written before 2026-09-12 uses the
old Ada* names; without this table a stale citation looks like a reference to a deleted file.
Evidence: renames verified by sha256 (10 of 11 byte-identical; ReasoningBare.md = old
AdaDopamine_gsm8k.md minus a 2-line trailer). Scope: documentation only -- no result changed.
-->
**NOTE RENAME (2026-09-12). The `Note/` docs were renamed; CONTENT IS UNCHANGED.** Older
citations anywhere in this repo, in the offline `RoleAnswer/` workspace, and in commit
messages still use the left column. Resolve them with this table rather than assuming a
deleted file:

| Old name | Current name |
|---|---|
| `AdaDopamine_gsm8k.md` | **`ReasoningBare.md`** |
| `AdaDopamine.md` | **`Behaviour.md`** |
| `AdaptiveThinking.md` | **`ThinkingCurve.md`** |
...
| `mmlue.md` | **`MmlueRaw.md`** |
| `Todo.md` | **`TODO.md`** (case-only; matters on the Linux server) |

**Three traps this table does NOT resolve.** (a) **`Behaviour.md` is the former
`AdaDopamine.md`, NOT the long-absent `AdaDopamineBehaviour.md`** -- the latter was already
gone before this rename and was never renamed, so a citation to it is still unverifiable.
(b) **`AdaptiveThinking_bp.md` and `AdaThink.md` are likewise absent** and acquired no new
name. (c) **The frozen `docs/*.json` artifacts were deliberately NOT rewritten** -- their
prose still says `AdaptiveThinking.md` / `AdaDopamine_gsm8k.md`. They are hash-pinned
evidence in the P3 chain (`p3_amendment_05`, `p3_supp_amendment_0{1,2}`,
`p3_supp_result_20260830`), and editing them for a cosmetic rename would invalidate the
recorded SHA256. Read them through this table; do not "fix" them.

`Note/ReasoningBare.md` was additionally shortened by a 2-line trailing separator during the
rename. Every other file is byte-identical to its pre-rename version (sha256-verified), so
no stored number, table or frozen wording moved.

**Required reading before non-trivial changes:**
- `Note/ReasoningBare.md` — current state. 2026-05-30 起 GSM8K template 已對稱化、mask offset bug 已修,所有舊 Phase 1/2 數字與當前 pipeline 不可比。Phase 1b(現已改稱 Phase 1)的重做**已完成**,見 §4.1–4.8;Phase 2 (Plans A–H3) 結論**暫擱置**,須重跑。**+α 機制表述已改框 (2026-07-24)**：主錨從「焦慮 anxiety / VTA→IPN」改為 **over-wanting → 冲动 impulsivity (抢答) + 认知僵化 / 强迫性反复 compulsivity / perseveration (loop),锚 VTA→NAcc**(理由:+α 数据无回避/freezing,抢答率随 α 单调升 = 急着 commit;不做 mania 类比)。DA→焦虑文献降為次要旁證。**脚本字段名 `analyze_loop_anxiety.py` / `ANXIETY_PATTERNS` / 表格 "Any anxiety" 保留原名**(改名破坏 U 形复现),但按 §2.3「命名说明」读作「强迫性 over-checking」——**勿把 doc 表述改回焦虑框架**。
- `Note/ThinkingCurve.md` — 歷史 Phase 1–2 設計、Plan A–H3 動機 / 失敗分析、Yerkes–Dodson framing、EMA + 1-step-lag physics(設計思路仍有效,但數字本身已過時)
- Literature & dopamine-LLM mapping now lives in the `Note/Behaviour.md` / `Note/ReasoningBare.md` family below (the earlier standalone `Dopamine.md` / `Dopamine_EN.md` / `Dopamine2.md` are gone). Other on-disk docs not indexed here: `Note/LogitsLens.md` (logit-lens directional analysis), `AdaptiveThinking_bp.md` (pre-eot Phase 1–2 backup; **NOT on disk** — historical pointer only, and it predates the 2026-09-12 Note rename so it never acquired a new name).
- `AdaDopamineBehaviour.md` — **NOT on disk as of 2026-08-11**, and referenced by older notes; treat citations to it as unverifiable until it reappears. It is an OLD name that was never renamed, because the file was already gone before the 2026-09-12 Note rename — **it is NOT the same file as the current `Note/Behaviour.md`** (which is the former `AdaDopamine.md`). Clinical behavioral signatures of dopamine excess vs. deficit (anhedonia / loss of drive / bradykinesia). Grounds the two-arm framing: α positive → over-wanting; α negative → under-wanting (§2.3). Reference for the behavioral-economics suite below, not a pipeline doc. **NOTE (2026-07-24): the +α arm was reframed in `Note/ReasoningBare.md` from "anxiety" to impulsivity + compulsivity/perseveration (VTA→NAcc) — that absent doc's §2.2 still uses the older anxiety wording; unify the framing before citing it, and see the `Note/ReasoningBare.md` doc-map note above.**
- `Note/Behaviour.md` — **behavioral-validation stage of the four-part research line** (RSN → behavioral dopamine → brain dopamine → thinking curve). The source of truth for the wanting-proxy experiments (§3 MCQ/GSM8K/MATH cross-model tables; §4 = the full ⑤–⑩ experiment catalog: Betting / Bandit / Pressure / Effort-choice, plus the *skipped* PIT / Reversal / Agentic / TRAIT with documented why-skipped reasons). When working on any behavioral-economics entry-point, read the matching §4.x here for the design rationale and prior results. **Note its GSM8K numbers are pre-eot** (peak α=−4) and superseded by `Note/ReasoningBare.md` (peak α=−6); unify before citing.
- `Dopamine_backup.md` — **the original (first-version) experiment record.** `Note/Behaviour.md` is the curated current doc; `Dopamine_backup.md` is where the *raw prior results* live that the newer docs don't repeat — notably the full Bandit Assistant-Role-vs-No-Role tables (§4.7), the Agentic/ScienceWorld §4.9 data (success%/penalty% per α), and the complete why-skipped diagnoses for Reversal (§4.8, position-bias + phasic-DA incompatibility), PIT (§4.10), Agentic (§4.9) and TRAIT (§4.11). Read it when you need a prior result that `Note/Behaviour.md` only summarizes. Its GSM8K/MATH numbers are pre-eot (see the note on `Note/Behaviour.md`).
...
- `Note/ThinkingCurve.md` §3.1.6 / Phase 1b is the signal-proxy trajectory state-effect analysis (**complete**; "Phase 1b" is a retired name — see §Offline analysis workspace); `AdaThink.md` is an auxiliary trace analysis outside the main line.
- `Note/Bandit.md` — **the Bandit design + literature doc** (renamed 2026-08-11 from `BanditExperiment_LiteratureReview.md`). §1.3 `Prompt Design Constraints` / `PV9 Prompt Modifications` is the design source for the pv9 Stage-1 changes; §2 is the literature review (EVOLvE, greedy-agent papers, Krishnamurthy et al.).
...
- **Two SELF-CONTAINED experiment subtrees were added 2026-09-03/04 and are deliberately isolated: `zebralogic/` (8 files) and `proofwriter_owa/` (10 + 12 tests).** Each holds its own loader, runner, scorer, launcher and tests, and touches **no** existing runner, mask, loader or result tree — that isolation is the design, not an oversight, so do not "integrate" them into `template.py`, `get_answer_regenerate_*.py` or the shared `TASKS` loop. Two consequences worth knowing before you go looking: **ProofWriter's pre-registration is `proofwriter_owa/PREREG_PROOFWRITER_OWA.md`, NOT under `docs/`** (every other protocol is in `docs/`; this is the one exception), and **both subtrees need `python3.10`** — their tests import numpy through the runner and a bare `python3` fails with `ModuleNotFoundError` partway through, which reads as a broken suite rather than a wrong interpreter. Verified passing 2026-09-04: `python3.10 zebralogic/test_zebralogic.py` and the 12 files in `proofwriter_owa/tests/`. **Their launchers are NOT symmetric either: `zebralogic/run_zebralogic.sh` is INSIDE the subtree, while `run_proofwriter_owa.sh` sits at the repo root** beside every other `run_*.sh`, so a root-relative guess finds one and misses the other. **Neither line is a fixed-workpoint transfer test** — both run their OWN 4-point dose exploration and never read the frozen GSM8K workpoint, so their results may not be added to the transfer scoreboard. **Both lines are now COMPLETE + CLOSED** (ProofWriter: COMPLETE + TERMINATED, 2026-09-05; ZebraLogic-Easy: COMPLETE + CLOSED, 2026-09-05) — see the status table for each line's own result before citing either; do not conflate their protocols or results.
...
3. **`llms.VicundaModel`** loads the LM. **Model-path convention (repo-wide): use the Hugging Face repo ID directly by default** — `meta-llama/Llama-3.1-8B-Instruct` and `Qwen/Qwen2.5-7B-Instruct` — and let Transformers use the HF cache. Do not invent a filesystem model path or write placeholders such as `<model目录>` in run commands. Use a local model directory only when that specific launcher or protocol explicitly records that the model was downloaded there. This is separate from `--mask_path`: masks are local files under `${BASE_DIR}/mask/`, e.g. Qwen uses `${BASE_DIR}/mask/qwen2.5_non_logits/nmd_0.5_16_22_7B.npy` (server default: `/data1/paveen/Dopamine/components/mask/qwen2.5_non_logits/nmd_0.5_16_22_7B.npy`). Prefer the existing launcher's defaults; if giving an expanded command, use these real values rather than `<mask目录>` or another unresolved placeholder. Three loading paths: `dream` diffusion (`AutoModel`, see `diffusion.py`), Mistral3 multimodal (`Mistral3ForConditionalGeneration`), or default `AutoModelForCausalLM`. All use bf16 + `device_map="auto"`. `_find_decoder_layers()` is the abstraction used by every layer-injection hook. **Generation entry points**: `VicundaModel.generate(inputs, batch_size)` for batched no-hook runs (`get_answer_*` uses this; pads with `padding=True`); `VicundaModel.generate_one(prompt, ...)` for bs=1 with caller-managed hooks (`track_hidden_states`, `closed_loop_gsm8k`, `track_dopamine_signal` all use this). **Do not mix**—batched vs bs=1 generation differs by ~2% acc on Llama due to padding artifact. `VicundaModel.regenerate(...)` (prefill-only steering path) takes an optional `stop_strings: list[str] = None` — default None keeps every existing caller byte-identical; only on the prefill_only branch, and only when set, does it pass `stop_strings`+`tokenizer` to `model.generate`. **Caveat learned the hard way:** HF `stop_strings` halts on the substring appearing *anywhere* in the output, so if a prompt's format spec literally contains the stop token (CGT's system prompt has `</choice>`), the model echoing it mid-reasoning truncates before the real answer — CGT tried `["</choice>"]` and invalid_rate jumped 0.02→0.11, so it reverted to natural EOS. Only use `stop_strings` when the marker can't appear in the prompt. `regenerate` also takes `prefill_tail_len: int = 1` (inject into the last N prompt tokens instead of only the last; default 1 = byte-identical to every existing caller, prefill_only path only — see the CGT Injection mechanics bullet).
...
- `--plan {none,static,A,B,C,D,E,F,G,H1,H2,H3}` selects controller (case-sensitive; see `Note/ThinkingCurve.md` §3.2)
...
> **Status: complete** for §4.1–4.8 of `Note/ThinkingCurve.md`. The naming is settled as Phase 1 = observation / Phase 2 = control; "Phase 1b" survives only in older notes. The collection tooling below is still the way to gather new signal data.
...
Evidence: ThinkingCurve.md §5.7; Qwen α=0 at-risk n=12/300.
...
Evidence: RoleAnswer/qwen_signal/logit_family_RESULT.txt; ThinkingCurve.md §5.5 Result 2-3.
...
**Dropped from the main line (2026-06).** Capitulation was moved out of the wanting/dopamine narrative: it measures PFC-level goal-maintenance under pressure (an indirect DA downstream), not tonic-DA wanting; the "capitulate vs. hold" readout is bidirectionally interpretable; it conflates with instruction-tuning sycophancy; and steering only works on Llama (Qwen≈0%, Mistral reverses). See `Note/Behaviour.md` for the rationale. Code/script are kept for reference but not part of the active suite.
...
- `get_answer_gpqa_bet.py` (`run_gpqa_bet.sh`, also drives `run_mmlu_bet.sh`) — **confidence betting**: model bets 0/2/5/10 points before answering. Mean bet / fraction bet=0 = wanting proxy. Out: `${BASE_DIR}/${MODEL}/gpqa_bet/`. **`run_gpqa_bet.sh` was extended (2026-06-16) to a full −8→+8 scan** (CONFIGS holds the 8 steered cells; orig=α=0 via the orig branch) — betting's readout is mean_bet, a SATURATING-MONOTONE quantity (no overload collapse; wanting rises until the bet=10 ceiling), so the scan confirms monotonicity + locates saturation, giving a second "needs-engagement → positive-α" curve for the motivation-knob argument. (**This originally read "alongside Bandit" — that pairing is withdrawn as of 2026-08-05**: the old Bandit +2 peak came from the voided pre-fix data, and the rebuilt pv6 Bandit shows no peak at all over −4/0/+4, only +α damage to post-discovery persistence. See `Note/Behaviour.md` §3.2.5 / §3.4.) **This is the deliberate exception to the "betting no alpha-scan" stance below** — that stance was about *inverted-U peak-finding* (betting is single-step, can't show right-arm overload); a monotone-saturation scan serves a different purpose. GPQA (n=646, already chat) carries the scan; MMLU stays ±4 (large-n dissociation, no curve).
  ...
  - **Betting is NOT byte-reproducible and this was deliberately NOT fixed.** `get_answer_gpqa_bet.py` runs `temperature=1.0` with **no `manual_seed` anywhere** (unlike the bandit/CGT family, which seed per `(run, round)`). Every re-run resamples: trends replicate, exact values do not. Consequence for doc maintenance — after ANY betting re-run, `Note/Behaviour.md` §3.1.1 / §3.1.2 tables must be updated to the new values; a small delta is the expected behaviour, not a regression to investigate. Adding a `--seed` is a small change (it would not disturb existing callers if defaulted to `None`) and is the obvious next step if betting goes into a paper, but as of 2026-07-29 it is **unresolved** — do not assume a stored betting number can be regenerated.
...
- `get_answer_regenerate_truthfulqa_gen.py` (`run_truthfulqa_gen.sh`, loader `data_truthfulqa_gen.py`) — **TruthfulQA-Generation = open-ended over-generation probe.** Free generation (bare, greedy, 128 tok), so it avoids both SIQA's accuracy-only null and HaluEval's Yes/No default-action trap: there is no binary default to coast on, so α expresses on *what / how much / how boldly* the model asserts. n=100, α=0/±4. **Results: `Note/Behaviour.md`.** Key points:
...
- **Gain-coordinate (V1) Phase 1 signal analysis — the CURRENT main line for §4 of `Note/ThinkingCurve.md`** (supersedes the V0 `x_t`/prefill-seeded-EMA scalar scripts). All read `dopamine_signal_*` + `metrics_*` from `llama3/dopamine/signal/`, `python3.10`:
...
- `analyze_cgt_seq.py` — **AUTHORITATIVE CGT-SEQUENTIAL (delay-aversion) metric panel.** `--asc llama3/cgt/seq_asc_v2b --desc llama3/cgt/seq_desc_v2b` → 4-LAYER per-condition table + KW/Spearman/**paired Wilcoxon** + cross-condition DAI with a **paired bootstrap 95% CI**. Layers: **PRIMARY** (mean_accept_step / `valid_only_accept_step` [= same value, explicit "trend ≠ invalid-survivor" proof] / early_stop_rate / accept_step1_rate / `max_bet_first_stop` [desc-only impulsivity disambiguator] / forced_lock_rate); **PERFORMANCE** (`final_score` / `mean_phase_score` — inverted-U, desc peaks +4 / asc peaks −4, but report qualitatively only); **BETS** (mean_bet / max_bet_rate / risk_taking — within-condition only); **KNOWING/DIAGNOSTIC** (qdm / invalid_rate / `empty_gen_rate` [−α 垮/沉默] / `color_confusion_rate` [Wait/Accept leak or empty color step] / color_blank_rate / `exact_bet_rate` / `dirty_bet_rate` / `dirty_valid_rate`). **TWO over-steer gates** (auto-flagged, EXCLUDE from fit): `invalid≥0.15` (−α empty/collapse) OR `dirty_valid≥0.15` — the latter catches the +α failure invalid_rate MISSES: at +8 the parser still extracts Accept/Wait but ~44% of valid replies are a paragraph / "Wait Accept" / probability restatement, not a bare one-word decision. `_bet_is_clean` judges EACH tier reply separately (a legit Wait→Wait→Accept chain is clean — do NOT concatenate across tiers). Display labels disambiguate the two "early" metrics: `early(step<=2)` (=early_stop_rate) vs `step1_accept` (=accept_step1_rate); the doc table's "asc/desc early" column = `step1_accept`. DAI section reports BOTH `DAI(bet)=mean_bet_desc−mean_bet_asc` (canonical CANTAB index, use this) and `DAI(step)` (timing-pure but ceiling-confounded on +α — do NOT report). **PAIRING IS LOAD-BEARING (fixed 2026-08-19).** `get_answer_cgt_seq.py` uses `seed=run_idx`, so run *i* faces a byte-identical chest sequence in EVERY α cell, and the same index pairs asc with desc (verified per-round on the stored v4 data). The unit of inference is therefore the **run (n=20, paired)** — same house convention as PV9/PV10. The pre-2026-08-19 build used an UNPAIRED Mann-Whitney; **its p-values are void.** Re-running flipped five cells (desc `mean_accept_step`/`mean_bet` at −8, desc `final_score` at +6/+8 → n.s.; asc `qdm` at −2 → sig) and left every clean-range primary result significant. The two −8 flips are a CORRECTED FALSE POSITIVE, not lost power: −8 has `invalid≈0.99`, so only **7 of 20** runs yield a value (asc: 0), and MWU was comparing 7 survivors against 20 — `_paired` now reports that `n` explicitly. `_paired` returns `(p, median Δ, n_pairs)` and drops NaN pairs. Reads the `e55b132`/v2b/v3/v4 JSON schema (pre-2026-08-19 runs have NO `forced_lock` field → treated as not-forced, which is why `forced_lock_rate` reads a constant 0 on all legacy data; empty-records cells emit all-NaN so the full sweep's dead −8 cells don't crash it). **Table口径 = valid-only** (`valid==True` filter); a prior doc table mistakenly INCLUDED invalid-fallback rounds (random accept_step/color), which made −8 look like a fake early-commit signal (step≈2, qdm≈0.527 = pure fallback noise) — always recompute valid-only.
  ...
  Evidence: analyze_cgt_seq.py qdm_major_* / asym_gradient; Behaviour.md §3.3 QDM 成分分解.
...
Evidence: ThinkingCurve.md §4.4 Result 4 (quadratic R²=0.352 vs linear 0.147, fitted peak ≈−1.9).
...
- `analyze_slow_state_behavior.py` — **TODO §2 Slow-State Behavioral Validation** (`Note/ThinkingCurve.md` §4.7). Pools 11 conditions × 300; features (`s_t`) and behaviors (commit step, gen_len, premature, loop, eos_fail) come from the SAME signal-JSON object, index-aligned. `--part descriptive|regression|heldout|plot|all`; `python3.10 -u`.
  - **Modeling-vs-finding split (mirrors ThinkingCurve.md §2.2 命名約定):** the `s_t` SLOPE remains the operationalization of the ramping/vigor hypothesis. This script asks only whether THIS task elicits the predicted association — **a null here is a task-level finding, NOT a construct downgrade or rename.**
...
Evidence: ThinkingCurve.md §4.8 Declaration marker 段; plot_sample_traj.py merge rule.
...
- **Cross-model Qwen2.5 GSM8K** (2026-08-20). `run_gsm8k_qwen25.sh` + `check_gsm8k_qwen.py` — its own launcher for the same reason as Mistral's (Llama's `run_gsm8k.sh` plain/pushy/4-role/182-backfill matrix is Llama-specific and frozen). This is a **direct replication** of the Llama dose curve, not a redesign: same 300-question benchmark, same driver, same templates, neutral / plain / greedy / `max_new_tokens=768` / bs=24. Four Qwen-specific facts, none inherited: layers **16–21** written as the exclusive `16-22` (**L=6**; Llama's `11-20` is L=9 and does NOT transfer), mask `nmd_0.5_16_22_7B.npy`, `size=7B` (28 decoder layers), and Qwen's `<|im_end|>` + LIST `generation_config.eos_token_id` (already unioned by `_build_terminators()` — verify, do not "fix"). Steps: `--check` (technical pre-flight only — model/tokenizer/mask/band/paths/fires; makes **no** judgement about results and has no pass/fail-then-block gate), `--baseline` (α=0 → the FINAL No-CoT `mdf_0`, read its raw output by hand), `--nocot` (the other 8 α; deliberately excludes α=0 so `mdf_0` is never re-run or overwritten), `--cot` (−4/0/+4), `--cot6` (−6/+6, extends that same curve). **Every α of one curve must stay on ONE card** (bf16 greedy is not byte-reproducible across GPUs); No-CoT and CoT write different `--ans_file`s (`answer_mdf_gsm8k` / `_cot`) so the two curves can run on two cards in parallel.
  ...
  - **Qwen Native Chat reporting rule (ReasoningChat).** The `−8/0/+6/+8` targeted comparison is scored with the frozen offline extractor: `first_acc` is primary and `last_acc` is sensitivity only. Inline `correct`/`correct_neutral` fields are run-health/provenance diagnostics, never manuscript performance. The recorded inline/offline checks are GSM8K `47.33/47.33`, `63.33/63.33`, `94.67/94.67`, `93.00/93.00` and MATH `71.67/71.33`, `71.33/70.67`, `73.33/72.67`, `72.00/71.33` for `−8/0/+6/+8`; GSM-Hard has no inline field. Retain the Qwen per-task Holm families (`m=3`) separately and do not pool Native Chat, Matched-Anchor, or Bare samples.
  ...
  Evidence: ReasoningBare.md Table 4.3b; RoleAnswer/backup/qwen_step2/commit_columns.py.
...
- **`record_template` mislabels GSM8K role templates.** It logs `templates["default"]` (with honest) for non-neg roles, but the code actually constructs the `neg` prompt (no honest). The `template` field in result JSON is therefore unreliable for GSM8K roles — trust the code path, not the logged string.
  ...
  Evidence: RoleAnswer/wps_region.py (near-optimal regions); ReasoningBare.md 1.2 / 2.3.
...
> **`Note/Manifold.md` is the results document — full tables, interpretation and claim boundaries live there and are NOT duplicated here.** This section is the implementation, provenance and runbook: what was run, what guards it, and which failures are already paid for.
>
...
**§3 RESULTS — full tables in `Note/Manifold.md` §3.** Runbook facts only: 36 bases in 65s; `stored_layer_indices=[10..18,31]`, `n_middle=9`; fit-phase rows prefill/`decode_all` m=185, `pre_commit` m=2940 (nq=**150**), `post_commit` m=3576 (nq=183) — `pre_commit`'s nq is lower because a sample committing at decode step 0 has a post window but no pre window.
...
Evidence: Manifold.md 3.2; 20 isotropic draws matched on m/nq/dim and per-question weighting.
...
Evidence: Manifold.md 3.3; manifold_prefill_exact.py, validated against a synthetic 25% split.
...
Evidence: Manifold.md 3.4; manifold_prefill_direction.py, validated on synthetic pure-scalar and orthogonal cases.
...
Evidence: Manifold.md 3.5-3.6; RoleAnswer/manifold/PREREG_incremental.md, PREREG_negative_arm_confirm.md.
...
Evidence: Manifold.md 3.6; RoleAnswer/manifold/PREREG_negative_arm_confirm.md and its addendum.
...
Evidence: Manifold.md 3.7; RoleAnswer/manifold/PREREG_decode_minimal.md.
...
Evidence: Manifold.md 3.8; prefill_{exact,direction}_qwen.json; PREREG_qwen_prefill.md failure conditions.
...
Evidence: Manifold.md §3.9; xlayer/direction_{llama_s0..s8,qwen_s0..s5}.json; PREREG_cross_layer.md.
...
Evidence: Manifold.md 3.10 + S4; recomputed from the frozen xlayer/exact_*.json, no new data.
...
Evidence: ThinkingCurve.md 5.8.0; thinking_curve/extract_metrics.py docstring.
...
> **`Note/ReasoningBare.md` §5.1–5.4 is the results document** — 口径, tables, evidence level
...
tables and their CIs. Narrative version: `Note/ReasoningBare.md` §5.1–5.4. The
...
Evidence: ThinkingCurve.md 5.8.1 footnote; early_candidate_detector.py (frozen 2026-08-21).
...
Evidence: ThinkingCurve.md 5.8.1; thinking_curve/extract_metrics.py.
...
**P1 cross-model Thinking Curve is RUN (2026-08-28): the behavioural difference is located in COMMITMENT TRANSFER, not entry gain.** Written up as `Note/ThinkingCurve.md` **§5.8** (renumbered from §5.9 when §5.7/§5.8 were merged into `5.7 Cross-Model Synthesis and Evidence Boundaries`). Offline in `RoleAnswer/thinking_curve/` (`extract_metrics.py`, `curves.py`, `fig_p1_commitment.png`), `python3.10`, no server — frozen extractors (`all_hash` / `norm_gsm8k` / `fallback_gsm8k` / `has_early_candidate`) are IMPORTED, never reimplemented. **Pairing is BY ORDER and that is verified, not assumed**: all 20 cells of both models hold the same 300 questions in the same order (so the comparison is item-matched across models too), asserted per cell at load with a fail-closed check.
  ...
  Evidence: thinking_curve/extract_metrics.py; ThinkingCurve.md 5.8.0/5.8.1.
  Scope: every commit-state number, both models. -->
...
> **`Note/ReasoningBare.md` §5.5 is the results document.** This block is provenance,
...
Evidence: ReasoningBare.md §5.5; p3/commit_panel_gsm_hard.py.
Scope: every GSM-Hard posN claim. -->
- **Commitment panel (`p3/commit_panel_gsm_hard.py`, `Note/ReasoningBare.md` §5.5) is
...
Evidence: docs/p3_supp_amendment_01.json; ReasoningBare.md §5.5.
...
- **LLAMA GSM-HARD IS TRUNCATION-BOUND: 91-96% cap-hit in all five cells, median decode
  ...
  **Cap-hit does not invalidate the paired P3 comparison as a FIXED-BUDGET estimand** —
  ...
  exact McNemar p=.53), and that covers that pair only. **A corollary for ReasoningBare.md §5.5's Llama
...
- **TWO PARTS, DIFFERENT EVIDENTIAL STATUS, and the split is the point.** Part 1 (the four
  ...
  `p3_supp_predictions.json` was frozen. Part 2's No-CoT half (`Note/ReasoningBare.md` §5.5's commitment
...
### RESULT (2026-08-30) — `Note/ReasoningBare.md` §5.6 is the results document
...
- **EXPLORATORY: llama's answer-first pattern.** Under CoT α=−6, **91 of 156
  ...
  α=0/−6 reads 11.3%/58.3%. This is ReasoningBare.md §5.5's premature-lock-in signature
...
> `docs/p4b_amendment_01.json`. **`Note/ReasoningBare.md` §5.8 is the results
...
> **`Note/ReasoningBare.md` §5.8 is the results document.** This block is
...
> **`Note/ReasoningBare.md` §6.1/§6.3 is the results document** (Table 6.1's
...
independently-registered CoT exploratory follow-up on this same task, and the
`CRUXEval-O Chat interface` status-table line above (2026-09-08) for the
llama3-only bare-vs-chat interface check that followed it — a `{0,−6}`
single comparison plus independent Chat-CoT and Chat-No-CoT four-point
sweeps, all null, none superseding this section's frozen Bare result.

## `cot-transfer-followup-v0`: explicit-CoT exploratory follow-up on the three P4/P4b/P4c boundary tasks

> **`Note/ReasoningBare.md` §6.3 is the results document** (merged No-CoT/CoT table +
...
**Implementation (added to this file 2026-09-04 — the code existed for a week with
no entry here).** Three runners, one per task, each a **FORK of its No-CoT sibling**
rather than a flag on it: `get_answer_{bbh_numeric,cruxeval,logiqa2}_cot.py`, driven
by `run_{bbh_numeric,cruxeval,logiqa2}_cot.sh`. Forking is the same choice P3-supp
made and for the same reason — the No-CoT runners produced FROZEN cells, and adding
a branch to them would put frozen and follow-up output on one code path. All three
define the cue as `COT_CUE = "Let's think step by step.\n"`, byte-identical to the
GSM8K/MATH/P3-supp cue, inserted immediately BEFORE each task's existing
final-format instruction, so the anchor token and every commitment feature keep the
meaning they had when the predictor was fitted. Scoring reuses the No-CoT
evaluators unchanged; `eval_cot_transfer_followup.py` is never edited by this line,
and `combine_cot_followup_holm.py` holds the **m=6** family and refuses to report
unless all six (task, model) pairs are present — a partial family would otherwise
print an m<6 adjustment under an m=6 label, the same failure P4's launcher guards
against.
...
```bash
# ZebraLogic-Easy and ProofWriter-OWA are BOTH COMPLETE + CLOSED (2026-09-05).
# These checks are local plumbing/format verification only, unrelated to
# either line's closed status. For ProofWriter, they exercise the LEGACY
# v0/v1 run_proofwriter_owa.sh launcher only, kept for historical reference;
# the closing v2 formal sweep ran through the separate
# run_proofwriter_owa_v2_preflight.sh, which carries no suspension guard.
# Both subtrees are self-contained and BOTH REQUIRE python3.10: their tests
# import numpy through the runner, so a bare python3 dies with
# ModuleNotFoundError PARTWAY THROUGH, which reads as a broken suite rather
# than a wrong interpreter.
python3.10 zebralogic/test_zebralogic.py
for f in proofwriter_owa/tests/*.py; do python3.10 "$f" || echo "FAIL: $f"; done
# The two launchers are NOT in the same place: ZebraLogic's lives INSIDE its
# subtree, ProofWriter's (legacy v0/v1) at the repo root beside every other
# run_*.sh.
bash -n zebralogic/run_zebralogic.sh && bash -n run_proofwriter_owa.sh

# The legacy v0/v1 launcher's suspension guard is a HISTORICAL protection
# mechanism from when the line was blocked at its alpha=0 feasibility gate
# (2026-09-04) -- it is enforced IN THE LAUNCHER, not only in the docs, so it
# is a SEMANTIC guard bash -n cannot see. It is unrelated to the line's
# current CLOSED status and is exercised here only to confirm the legacy
# script itself has not silently changed; it does NOT need to be re-armed or
# repurposed. All four blocked steps must exit 2; validate-data/preflight/
# analyze are deliberately still allowed.
for s in canary pilot llama-sweep qwen-sweep; do
  bash run_proofwriter_owa.sh "$s"; echo "$s -> $?"   # each must print 2
done
# Verified passing 2026-09-04: zebralogic 1 file, proofwriter 12 files, and all
# four blocked steps refusing with exit 2.
