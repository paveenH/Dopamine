...
```bash
# ZebraLogic (the ACTIVE line) and ProofWriter-OWA (COMPLETE + CLOSED
# 2026-09-05 -- these checks exercise the LEGACY v0/v1 run_proofwriter_owa.sh
# launcher only, kept for historical reference; the closing v2 formal sweep
# ran through the separate run_proofwriter_owa_v2_preflight.sh, which carries
# no suspension guard). Both subtrees are self-contained and BOTH REQUIRE
# python3.10: their tests import numpy through the runner, so a bare python3
# dies with ModuleNotFoundError PARTWAY THROUGH, which reads as a broken
# suite rather than a wrong interpreter.
