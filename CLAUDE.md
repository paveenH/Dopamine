...
| CC | unconfident | +4 | 27.42% | +27.10 pp | 0.02% | -99.38 pp |
  ...
  **Reading, both directions must be reported together, never one alone**: positive-α CC steering strongly suppresses E-option abstention on the `unconfident` prompt (99.39%→9.04%→0.02% at α=0→+2→+4) — the manipulation-check direction the mask should encode — but it ALSO degrades task competence on the `confident` prompt (accuracy 65.20%→56.99%→32.39%, `wrong_non_E` correspondingly rising) and does not restore genuine correctness on `unconfident` (its own `wrong_non_E` rate rises just as sharply as its accuracy — the large `unconfident` accuracy jump is substantially a side-effect of no longer defaulting to a guaranteed-wrong E, not evidence of improved knowledge). **α=+4 is over-steering** (both roles converge toward a shared, degraded, mostly-wrong-non-E regime); **α=+2 is the more usable dose** (E-ratio already collapsed on `unconfident`, competence damage on `confident` present but far smaller). Do not cite the `unconfident` accuracy gain alone as "CC improves confidence-linked correctness" without the paired `wrong_non_E`/`confident`-accuracy caveat.
