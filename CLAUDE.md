...
- Output dir: `${BASE_DIR}/hidden_states/${TASK}/${RUN_TAG}/hs_<task>_<size>_<mode>_<role>_L<s>-<e>.h5` (selective storage ≈ 2–3 GB per role, gzip fp16). `run_track_hidden_states.sh` isolates each collection under `RUN_TAG` and refuses to truncate existing HDF5 unless `ALLOW_OVERWRITE=1`.
