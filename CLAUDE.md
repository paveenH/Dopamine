...
| Reasoning RSN (RRSN) history (old 3-task forced-answer version, and its GSM8K-abstention-role redefinition) | **BOTH VERSIONS RETIRED FROM THE MAIN LINE, 2026-09-18 — do not reintroduce either as an active comparison target, and do not reuse the name RRSN for any current direction without a new explicit instruction.** (1) The ORIGINAL RRSN (unweighted mean of the 3 per-task GSM8K/MATH/GSM-Hard Expert−Non-expert `role_diff`, built from FORCED-ANSWER prompts with no abstention option) backed `6.reasoning_rsn_morphology/`, `7.mrsn_rrsn_exact_nmd/`, `8.rrsn_gsm8k_positive_control/`, `gsm8k_rrsn_positive_control/`, `cross_steering_reasoning/`, and the `RoleAnswer` `gsm8k/rrsn`/`mmlue/rrsn` result trees — all code and derived results deleted. (2) RRSN was then REDEFINED to mean the GSM8K-abstention-role ARRSN direction (`gsm8k_abstention_expert_mean − gsm8k_abstention_non_expert_mean`, n≈1019 construction split, an "honest" + abstention-enabled prompt), backing `9.arrsn_mrsn_dense_comparison/`, `10.arrsn_mrsn_exact_nmd/`, `extract_gsm8k_abstention_role_hs*.py`, `mean/mean_gsm8k_abstention_role*.py`, and a fixed-300-question NO-STEERING behavioral audit comparing expert vs non_expert abstention rates — this line is ALSO retired; all code and derived results deleted. **Key negative result from the behavioral audit before retirement: on the fixed 300-question benchmark, with strict abstention scored as `generated.strip() == "I am not sure"` (exact whole-reply match, not substring), BOTH Llama-3.1-8B and Qwen2.5-7B recorded 0/300 strict abstentions in BOTH the expert and non_expert role — the model essentially never produces a clean, bare abstention under either role framing; "I am not sure" phrases that did appear were almost always followed by continued reasoning or a `####` answer, not a stop.** The GSM8K abstention-enabled Role HS extraction (`extract_gsm8k_abstention_role_hs.py`) and its construction-split sibling were purely this line's infrastructure and were deleted with it — they were never used by anything outside this line. The Chat–Bare pipelines and the `2.constructed_directions/` directory (including its 3-task `role_diff_*` arrays, still read by the kept `3.dense_direction_similarity` / `4.full_role_chat_direction` lines as a "Reasoning Role mean" reference point) were NOT touched — see the `Note/Neurons.md` caveat on citing that reference. All detailed run logs, hashes, and result tables that used to live in this history block have been deleted along with their data; do not follow any old path reference into this block, as none of it resolves any more. |
| GSM8K Role directions, full 1319-question set (`gsm8k_role` = **GRSN**, `gsm8k_role_abstention` = **AGRSN** — deliberately NOT named RRSN/ARRSN) | **NAMING (frozen 2026-09-21): GRSN = the `gsm8k_role` direction (expert − non-expert, no abstention exit, full GSM8K). AGRSN = the `gsm8k_role_abstention` direction (expert − non-expert, v2 abstention-enabled prompt, full GSM8K).** Both names refer ONLY to this full-1319-question line — they do NOT retroactively rename the retired RRSN/ARRSN results below (those stay RRSN/ARRSN, historical), and they do NOT rename MRSN, ChatSN, or general RSN. Both GRSN and AGRSN are planned to be constructed from the SAME full 1319-question set (not the fixed 300 alone) — see `Note/Todo.md` item 53. **CODE + LOCAL SYNTHETIC-FIXTURE TESTS COMPLETE, 2026-09-21; NOT YET RUN ON GPU (assistant does not run GPU jobs).** Two INDEPENDENT GSM8K Role hidden-state extraction lines, each over the FULL GSM8K test set (n=1319, not the frozen 300), prefill-only, no generation, no steering. Built per explicit instruction that the old RRSN/ARRSN names are retired and must not be reused without new explicit authorization (see the RRSN history entry above) — these are two plainly-named, independent directions. **`gsm8k_role`** = NO abstention exit, reusing `template.build_gsm8k_default_suite(cot=False, wording="plain")["neg"].format(character=utils.ROLE_TO_CHARACTER[role], context=question)` — the SAME routing `get_answer_regenerate_gsm8k.py` (line 66) and `extract_role_hidden_states.py` already use for GSM8K non-neutral roles, i.e. the existing Bare behavior-experiment prompt family (the `answer_mdf_gsm8k/mdf_0` tree), extended from 300 to all 1319 questions. **`gsm8k_role_abstention`** = the v2 abstention-enabled prompt, imported directly (`from get_answer_gsm8k_role_abstention_v2 import PROMPT_TEMPLATE, ROLE_EXPERT, ROLE_NON_EXPERT`), never retyped — the same prompt already run as a 300-question behavioral, no-steering pilot (see the "GSM8K role-abstention prompt v2" entries elsewhere in this table), now extended to all 1319 questions for hidden-state extraction. **Sample file**: `build_gsm8k_1319_full.py` (new, standalone) builds `components/benchmark/gsm8k_1319_full.json` by re-deriving the full `openai/gsm8k/main/test` set's OWN original index order (1319 rows, unfiltered) and cross-checking — not merely trusting — that it equals EXACTLY the union of the existing frozen 300-question file (`gsm8k_test_sample.json`) and the existing 1019-question construction split (`gsm8k_construction_split.json`); each row carries an `in_300_sample` boolean, which is a PROVENANCE/AUDIT field only and is NEVER used to filter, reorder, or select which rows are extracted — all 1319 rows are extracted unconditionally in both tasks. Fail-closed on: wrong file counts, internal duplicates in either input file, nonzero overlap between the 300-file and the construction split, a live-dataset row count other than 1319, and the union not exactly equalling the live full-test set (no extra, no missing question). **Extractor**: `extract_gsm8k_role_hs.py` (new, standalone; does NOT import or edit `extract_role_hidden_states.py`), mirroring that script's overwrite-guard/atomic-write/pairing-digest/model-shape-lock/fail-closed conventions but scoped to GSM8K only and parameterized by `--task {gsm8k_role,gsm8k_role_abstention}`. `--run_all` loads the model ONCE and runs both conditions (expert, non_expert) of the given task sequentially. Fixed global final-prefill-token expectation reused verbatim from `extract_role_hidden_states.py` (id=220, text=`' '`, both prompt families end in a literal `"...Answer: "` trailing space), asserted per-sample, not re-derived per cell. Output: `components/hidden_states/llama3/{gsm8k_role,gsm8k_role_abstention}/{expert,non_expert}_8B.h5` (+ sibling `.manifest.json`), each H5 additionally carrying an `in_300_sample` (bool, shape (1319,)) dataset. **Mean/diff**: `mean/mean_diff_gsm8k_role.py` (new, sibling of `mean/mean_diff_chat_construction.py`, same fail-closed/atomic-write/float32-accum-float64-audit conventions), direction frozen as `diff_mean = expert_mean − non_expert_mean` (matching this repo's other Expert−Non-expert conventions, e.g. `mean/mean_diff.py`'s `role_diff`). Computes the PRIMARY direction on the full 1319 rows, and — per explicit instruction ("分别报告全量 1319 题，以及原固定 300 题与其余 1019 题的方向一致性，作为稳定性检查，不用任何子集重新挑选方向") — ALSO computes, in-memory only (never persisted as separate `.npy` files, never substituted for the primary direction), the subset_300 and subset_remaining_1019 diff_means and reports their pooled + per-layer cosine similarity against the full-1319 direction and against each other, as a `stability_check` block in the output `manifest.json`. Output: `components/hidden_states_mean/llama3/{gsm8k_role,gsm8k_role_abstention}/{expert_mean,non_expert_mean,diff_mean}_8B.npy` + `manifest.json`. Fail-closed on: missing file(s), wrong `--size`, `n_samples≠1319`, shape mismatch, expert/non_expert shape or `ordered_sample_identity_sha256` mismatch, a `model`/`task`/`condition` H5 attr disagreeing with the file being read, `generation_performed≠False`, `prefill_only≠True`, `chat_template_applied≠False`, `steering_applied≠False`, non-finite values, `n_in_300_sample≠300` or `n_remaining_construction≠1019` (either condition), the two conditions' `in_300_sample` boolean arrays not being IDENTICAL (both were extracted from the same `gsm8k_1319_full.json` and must flag the same rows), and an existing output. **Launchers**: `run_gsm8k_role_hs_llama3.sh` (task `gsm8k_role`, intended for GPU 0) and `run_gsm8k_role_abstention_hs_llama3.sh` (task `gsm8k_role_abstention`, intended for GPU 1) — separate output trees, separate logs, each loads the model exactly once via `--run_all`, designed to run concurrently on two cards. **CAUTION FOR ANY FUTURE STEERING WORK ON THE 300-QUESTION SET, per explicit instruction**: because the full-1319-question direction construction INCLUDES the original 300 questions, any later steering evaluation run on those same 300 questions using a direction built from this `gsm8k_role`/`gsm8k_role_abstention` line is **NOT** an independent held-out test — the 300 questions already contributed to constructing the direction being evaluated. This must be stated explicitly whenever such an evaluation is reported, not left implicit. **Locally verified (no GPU): `py_compile`/`bash -n` clean on all five new files** (`build_gsm8k_1319_full.py`, `extract_gsm8k_role_hs.py`, `mean/mean_diff_gsm8k_role.py`, both launchers); prompt-construction logic for both task families independently reproduced and verified against the real `template.py` no-CoT `neg` string and the real `get_answer_gsm8k_role_abstention_v2.py` `PROMPT_TEMPLATE` (both end in the expected `"...Answer: "` trailing-space tail; both families' expert/non_expert prompts differ ONLY by the role-phrase substitution within that family). `mean/mean_diff_gsm8k_role.py` was run end-to-end against a synthetic (n=1319, 33-layer, 4096-hidden) H5 fixture on the local `python3.10` (h5py/numpy available there): the diff-identity check, the float32/float64 consistency audit, and the subset_300-vs-subset_remaining_1019-vs-full_1319 cosine `stability_check` all executed and produced a valid `manifest.json` + three `.npy` files; the overwrite guard correctly refused a second run against the same output dir; four fail-closed guards were individually mutation-tested against corrupted fixtures (`n_samples=1300` instead of 1319, mismatched `ordered_sample_identity_sha256` between expert/non_expert, and a shifted/mismatched `in_300_sample` boolean array between expert/non_expert) and each refused with the correct, specific error message before writing any output. **No GPU has been launched for this line yet.** |
| GSM8K construction-split Chat direction (`gsm8k_construction`, no protocol id) | **CODE COMPLETE + LOCALLY VERIFIED 2026-09-17; NOT YET RUN ON GPU.** Standalone GSM8K-only sibling of `extract_interface_hidden_states.py`/`mean/mean_diff_chat.py` (the existing 300-question, 3-task Chat/Bare interface pipeline whose `Chat–Bare transition-subspace analysis` line above documents the downstream use), built to use the SAME `components/benchmark/gsm8k_construction_split.json` (n≈1019) as the now-retired GSM8K abstention-role ARRSN/RRSN line so the two were sample-composition-matched — that comparison target no longer exists (see the RRSN history entry below), but the construction-split question file itself is unaffected and this Chat direction line stands on its own, independent of ARRSN's code or results. New code: `extract_interface_hidden_states_construction.py` (per-cell `--model {llama3,qwen2.5} --condition {chat,bare}`, GSM8K-only — no `--task` flag, no MATH/GSM-Hard) + `run_interface_hs_construction_{llama3,qwen25}.sh` (2 cells each: chat+bare) + `mean/mean_diff_chat_construction.py` (single-task sibling of `mean/mean_diff_chat.py`, `n` read from data not hardcoded to 300, cross-checked for chat/bare shape+pairing-digest equality). **Every prompt/model/layer-storage convention is copied byte-for-byte from the parent script** (same `MODELS` dict incl. `expected_tail_bare`/`expected_tail_chat` token id/text pairs, same `build_gsm8k_default_suite(cot=False, wording="plain")["neutral"]` body builder, same `apply_chat_template`+`strip_leading_bos` chat wrapping, same prefill-only all-layers-stored convention, same fail-closed final-token/no-double-BOS/finite checks) — the ONLY differences are: GSM8K-only scope, the construction-split question file (`n` cross-checked against `gsm8k_construction_split_manifest.json`'s own `construction_split_n` AND `construction_split_ordered_sample_identity_sha256`, dies on mismatch), and a separate non-colliding output tree `components/hidden_states/{model}/gsm8k_construction/{condition}_{size}.h5` (distinct from the parent script's `.../gsm8k/{condition}_{size}.h5`; the sibling `.../gsm8k_abstention_role_construction/` H5 tree this line once disambiguated against has since been deleted along with the rest of the retired RRSN/ARRSN line). Output mean/diff: `components/hidden_states_mean/{model}/gsm8k_construction/{chat_mean,bare_mean,diff_mean}_{size}.npy` (**per-model top-level directory, matching the existing `hidden_states_mean/{model}/` layout used everywhere else in this repo — NOT `{model}_chat_construction`**, since the `gsm8k_construction` task subdirectory alone already disambiguates from every other task) (fp16, `(1,1,n_layers,hidden_size)`, `diff_mean = chat_mean − bare_mean`, round-then-subtract-free — direct elementwise mean over the sample axis then subtract, matching `mean_diff_chat.py`'s own convention exactly, NOT the abstention-role mean scripts' float16-then-upcast round-then-subtract convention, since this pipeline never separately saves means before computing the diff). **`mean_diff_chat_construction.py`'s per-cell validation is LOCKED, not just shape/digest-based (tightened 2026-09-17 after review):** `--size` must match the model's own locked size (llama3=8B, qwen2.5=7B); `n_samples` must equal exactly 1019 (the locked construction-split count, not merely "positive"); each H5's own `model`/`task`/`condition` attrs must agree with the file actually being read (catches a misplaced/mislabeled H5, e.g. the 300-question tree copied into this path by mistake); `generation_performed` must be False and `prefill_only` True; `chat_template_applied` must match its own condition (True for chat, False for bare — catches a chat/bare mislabeling that would silently compute the diff backwards or from two files of the same condition); `questions_sha256` and `construction_split_manifest_digest` must agree between chat and bare, in addition to the existing `ordered_sample_identity_sha256` pairing check. The extractor's own manifest cross-check was also tightened: a MISSING `construction_split_ordered_sample_identity_sha256` key in the split manifest (not just a mismatched one) now correctly dies rather than silently passing (`if not manifest_digest or manifest_digest != pairing_digest`, was `if manifest_digest and ...`). **Locally verified (no GPU): `py_compile`/`bash -n` clean; `--print_pairing_digest_only` run against the real local construction-split fixture reproduces `n=1019` and digest `cb7f8bf6639e8b728ed6fa0b1af182c0c8f98df62a316d0cc6423d34bbf3c8b3` — matching the split builder's own recorded digest exactly, and identical across both models' launcher invocations (cross-model consistency of the construction question set); both `run_interface_hs_construction_{llama3,qwen25}.sh` driven with real arguments (not just `bash -n`) reach model-loading before terminating locally (no GPU available), confirming path resolution/digest cross-check/output-path construction all correct.** Mutation-tested (2026-09-17, each check individually, against a valid n=1019 fixture with one attr corrupted at a time): missing split-manifest digest key, `n_samples_done` mismatch, wrong `model`/`task`/`condition` attrs, `generation_performed=True`, `prefill_only=False`, `chat_template_applied` mismatched to its own condition, `questions_sha256` mismatch, `construction_split_manifest_digest` mismatch, `n_samples≠1019`, and wrong `--size` for the locked model — all 11 correctly refuse with a specific, distinct error message and write nothing; `mean_diff_chat_construction.py`'s overwrite guard also correctly refuses a re-run listing all 4 existing output paths. **Explicit interpretation-boundary caveat (from review, not a code issue): `Chat_direction = chat_mean − bare_mean` still confounds the native-chat-template wrapping itself with the accompanying change in final prefill anchor/token (bare ends on `Answer: `/token 220; chat ends on the assistant-header token, 271 for llama3 / 198 for qwen2.5 — see `extract_interface_hidden_states.py`'s own documented confound). This direction must NOT be described as a pure "Chat semantic direction" — it mixes template-wrapping semantics with injection-site/anchor-token geometry, same caveat the `GSM8K/MATH chat matched-anchor control` line below documents for a different pipeline.** **No GPU has been launched for this line yet — the assistant does not run GPU jobs.** |
...
| GSM8K role-abstention prompt v2 unified offline scoring (`gsm8k-role-abstention-v2`) | **BEHAVIOR-ONLY, NO STEERING; not a new RRSN.** The latest unified offline-score reconciliation compares the old Bare output `/Users/paveenhuang/Documents/RSNResult/RoleAnswer/AdaResult/llama3/gsm8k/bare/mdf_0/gsm8k_8B_answers_20_11_20.json` with v2 `/Users/paveenhuang/Documents/RSNResult/RoleAnswer/AdaResult/llama3/gsm8k/role_abstention/gsm8k_role_abstention_v2_8B.json`. Both contain the same 300 GSM8K questions in matched order (v2 records `question_text_sha256=64af9b38dac72f35ee1efa97bea8026e0c4d97b35730e76361eea9581a23cf35`; v2 has no populated `question_idx`, so the verified index-aligned question text is the pairing key). Bare asks `{character}` to provide `####`; v2 instead uses `an honest expert` / `an honest non expert` and adds the conditional instruction: solve → provide `####`, otherwise answer exactly `I am not sure` and provide no numeric answer. V2 generation is Bare/no-CoT with `VicundaModel.generate`, no diff matrices or steering, `temperature=0`, `top_p=0.9`, `max_new_tokens=768`, and batch size 24. **Scoring rule:** frozen `/Users/paveenhuang/Documents/RSNResult/RoleAnswer/analyze_first_last_acc.py` defines overall `first_acc` on all 300 items: first `####` when present, otherwise its production fallback chain; v2's original `/Users/paveenhuang/Documents/RSNResult/RoleAnswer/analyze_gsm8k_role_abstention_v2.py` reports `first_hash_acc` from the first `####` only, with no no-marker fallback. Latest unified results: Bare expert/non-expert `first_acc` 58.00%/68.00% (expert-only/non-expert-only correct 23/53; exact paired McNemar `p=0.00077`); v2 82.00%/74.33% (46/23; `p=0.0076`). The role-difference change is +17.67 pp (paired bootstrap 95% CI [+9.67, +26.00]); the v2 first-`####` contrast remains 71.67%/66.67%, previously `p≈0.124`. Strict abstention is 0/300 in both v2 roles. These statistics come from the latest offline unified scoring report; they compare whole prompts, cannot be attributed to the abstention outlet, and do not identify a new RRSN. |
