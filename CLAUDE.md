...
- **`####` wording matters — use `"Provide your final numeric answer after '####'."`** (the neutral, history-aligned wording). A pushier variant (`"Give your final answer as a single number after '####'."`) induced early-####抢答 (model commits an un-reasoned guess; expert 72% early-####, No-CoT acc collapsed to 34%). The pushy wording is kept conceptually as a future positive-control ablation (prompt wording vs α-steering as two levers on commitment timing). `extract_gsm8k_answer` takes the FIRST `####`, so an early 抢答 locks the wrong answer even when the correct one appears later — another reason to keep wording neutral. **Run matrix:** `run_gsm8k.sh` is now an **incremental top-up** (2026-06-03): `WORDING` defaults to `plain` only (set `WORDING=pushy` for the ablation, which writes to separate `_pushy` dirs — `answer_mdf_gsm8k_pushy` / `_cot_pushy` — so it never collides with the plain main line). Completed cells are commented out, not deleted: step [1] (α=0 No-CoT all roles) and step [2] (α=0 CoT neutral) are done; the live steps are [3] the **No-CoT dose-response sweep** `±2/±6/±8` neutral (±4 omitted — already in `mdf_4`/`mdf_-4`) and [4] CoT `±4` neutral (`--cot` → `answer_mdf_gsm8k_cot`). `run_math.sh` has **no** pushy variant — MATH uses the neutral `\boxed{}` directive only. NOTE: `analyze_first_last_acc.py`'s `GSM8K_DIRS` must gain `mdf_±2/±6/±8` + the CoT `±4` entries before the dose-response ACC can be computed.
