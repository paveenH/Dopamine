...
| MMLU-E explicit-confidence HS collection (`mmlue-confidence-hs-v0`) | **COLLECTION IN PROGRESS, 2026-09-08 — HS-ONLY, no analysis yet.** Llama3.1-8B-Instruct, `run_hidden_mmlue_confidence_hs.sh`, all 57 `detection/task_list.py` MMLU tasks, roles `confident`/`unconfident` ONLY (the ACLARR §app:prompt-template-confidence explicit-confidence anchors — "Now you are confident." / "Now you are unconfident.", uses the same explicit confident/unconfident role condition and existing template code path — `templates["neg"]` via `utils.construct_prompt`, which already special-cases any role name containing `"confident"` to drop "honest" and use the neg template). MMLU-E (`suite=default --use_E`, A–E, E="I am not sure"), bare-string (no `--use_chat`), no CoT. **This stage saves RAW HS + answers ONLY** — no mean_diff / NMD / PCA / overlap / manifold / steering. Deliberately isolated from the existing `run_hidden_mmlu.sh` (7-role MMLU HS collection) — separate launcher, separate output trees, neither touches the other's code or results. **Real paths (server-verified 2026-09-08, differ from an earlier draft that pointed at stale locations): MMLU per-task JSON at `${BASE_DIR}/mmlu` = `/data1/paveen/Dopamine/components/mmlu` (NOT the old RolePlaying tree); HS output at `${BASE_DIR}/hidden_states/llama3/mmlue_confidence/` (NOT `ConfSteer/HiddenStates`); answers were manually moved post-collection to `${BASE_DIR}/answer/llama3_confidence/` (the launcher's own default write target is `${BASE_DIR}/llama3/answer_hs_mmlue_confidence/` — a DIFFERENT path; this was a manual `mv`, not a script change, so cite the moved path for anything after collection and the launcher default only when describing what `run_hidden_mmlue_confidence_hs.sh` itself writes).** `get_answer_logits.py` gained one new arg for this, `--mmlu_dir` (defaults to `{base_dir}/mmlu`, so every existing caller incl. `run_hidden_mmlu.sh` is byte-unchanged). The launcher is fail-closed with THREE preflight checks run BEFORE the model loads and BEFORE any file is written — MMLU_DIR exists, every one of the 57 tasks has a JSON file (checked via a plain Python subprocess whose exit status is captured with an explicit fallback assignment, not a process substitution, because a substituted failure would leave the missing-list silently empty under `set -e`), both output directories are empty (no silent overwrite) — and `cd "${WORK_DIR}"` runs BEFORE the task-list check, since `detection.task_list` import assumes cwd = project root. Both `verify_mmlue_confidence_hs.py` and `mean/mean_diff_confidence.py` run on the SERVER (they read `/data1/paveen/...` paths and need the server's `h5py`/`numpy`), so invoke them with the server's `python`, **NOT `python3.10`** — `python3.10` is the local-analysis-box-only convention (per the `PY="${PY:-python}"` rule above) and does not exist on the server. `verify_mmlue_confidence_hs.py` is the acceptance/manifest tool: checks EVERY sample (not just the first) for both roles' `answer_/prob_/softmax_/logits_` keys, answer ∈ A–E, softmax/logits length exactly 5, all-finite; checks H5 shape is exactly `(N, 33, 4096)` (33 = 32 Llama3.1-8B decoder layers + embedding, per `output_hidden_states=True`) and dtype float16, scanning NaN/Inf per-sample (never `ds[:]` whole-file); a corrupt/missing file is recorded as a per-task failure and does not crash the run or silently pass; exits 1 if any task fails. Aggregate confident/unconfident accuracy + E-ratio are reported as a manipulation check only — **not used to filter or select neurons at this stage**. **`mean/mean_diff_confidence.py` (NEW, standalone, does not touch `mean/mean_diff.py`) computes the mean(confident_HS) − mean(unconfident_HS) matrix** — reads H5 directly (not per-role `.npy`, unlike `mean_diff.py`), keyed on `answer_confident`/`answer_unconfident` (unlike `mean_diff.py`'s `answer_{task}_expert`/`answer_non_{task}_expert` keys). **Sample selection is DIVERGENT PAIRS ONLY** (`answer_confident != answer_unconfident`), per the user's explicit choice — mirrors `mean_diff.py`'s inconsistent-sample restriction rather than using the full 300-question universe; do not silently switch to full-sample averaging without re-deciding this. Output: `components/hidden_states_mean/llama3_confidence/diff_mean_confidence_{size}.npy` (shape `(1, 33, 4096)` fp16) + a JSON summary (per-task divergent counts, skipped tasks, total samples used). This still does NOT build an NMD mask, filter/select neurons, or steer — it is one step further than collection, not the full pipeline. Downstream neuron-selection/NMD/PCA analysis lives separately under `~/Documents/RSNResult/RoleHidden/llama3_confidence_logits/` (a new subdir, local analysis box only). |
