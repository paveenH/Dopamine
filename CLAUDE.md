...
| ProofWriter-OWA 4-point exploration (`proofwriter-owa-v0`) | **CODE COMPLETE, NO GPU CELL RUN** — prereg lives at `proofwriter_owa/PREREG_PROOFWRITER_OWA.md`, **NOT in `docs/`** (the only protocol so placed; look there before assuming it is missing). Own 4-point dose exploration (llama `{−6,−4,0,+4}`, qwen `{−6,0,+6,+8}`), **no relation to the fixed-workpoint transfer line**. OWA is three-valued True/False/**Unknown** — do not read it as CWA. Prompt is OUR construction (v1 after a llama α=0 preflight measured 100% truncation / 96.7% parse failure at 1024). Code isolated under `proofwriter_owa/` |
