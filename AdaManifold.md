...
**Primary layer（Llama dec 18 / Qwen dec 20），TEST split：**

| model | cell | ‖d‖ | k=5 | enr | k=10 | enr | k=20 | enr |
|---|---|---|---|---|---|---|---|---|
| Llama | −8 | 24.63 | 9.6% | 78.4× | 17.0% | 69.4× | 21.4% | 43.7× |
| Llama | −6 | 17.67 | 9.5% | 77.6× | 16.8% | 69.0× | 21.2% | 43.5× |
| Llama | +6 | 13.23 | 5.8% | 47.9× | 8.1% | 33.1× | 9.8% | 20.2× |
| Qwen | −8 | 119.50 | 2.5% | 17.8× | 4.7% | 16.8× | 6.6% | 11.8× |
| Qwen | +6 | 89.11 | 1.7% | 12.3× | 3.4% | 12.1× | 4.7% | 8.4× |
| Qwen | +8 | 120.68 | 2.2% | 15.8× | 4.2% | 15.1× | 5.6% | 10.1× |
| Qwen | +12 | 181.33 | 1.7% | 12.4× | 3.7% | 13.2× | 4.8% | 8.7× |
...
在每一种 layer-matched 形式下（primary 对 primary、各自最佳层对最佳层），Llama 的 enrichment 都是 Qwen 的 **2.4–4.8 倍**，且方向在三个 k 上一致。因此这是一个**稳定的描述性差异**，可以写入结论。

**必须同时声明的边界。** 若把两模型的全部 layer×dose 值当作两个未配对的集合，取值范围是**重叠**的（k=20：Llama 6.1–55.9×，Qwen 4.7–18.3×）——Llama 的低层 enrichment 低于 Qwen 的高层。所以正确措辞是「**在 layer-matched 比较下 Llama 的 entry displacement 与 top-PCA natural directions 对齐更强**」，**不能**写成「Llama 处处高于 Qwen」。
